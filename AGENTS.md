...
| **New Vais front contract** | `bash scripts/test-vaisc-front.sh` | day-1 native subset과 print/putchar IO slice를 accepted source로 실행하고, bad helper signature/for/struct/Rust `&&`/list/string 등 unsupported 문법이 `help:` 진단으로 실패하는지 확인. |
