...
| **New Vais front contract** | `bash scripts/test-vaisc-front.sh` | native subset과 print/putchar, simple struct, payload-free enum/match, List push/len/index/sum slice를 accepted source로 실행하고, bad helper signature/for/payload enum/match/Rust `&&`/unsupported method/string 등 unsupported 문법이 `help:` 진단으로 실패하는지 확인. |
...
| **New Vais parity manifest** | `bash scripts/test-vaisc-parity.sh` | `tools/vaisc-parity.tsv`에 기록된 `native-supported` 예제를 New Vais `vaisc`와 Legacy oracle 양쪽에서 값 비교하고, `bootstrap-only`/`tracked` 상태가 stale하지 않은지 확인. |
