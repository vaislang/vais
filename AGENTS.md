...
| **New Vais front contract** | `bash scripts/test-vaisc-front.sh` | native subset과 print/putchar, simple struct, payload-free enum/match, small Int-coded payload enum/match, List push/len/index/sum slice를 accepted source로 실행하고, bad helper signature/for/broader payload enum/match/Rust `&&`/unsupported method/string 등 unsupported 문법이 `help:` 진단으로 실패하는지 확인. |
