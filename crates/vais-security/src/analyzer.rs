        ...
        for func in &block.functions {
            ...
            if matches!(
                ...
                self.findings.push(SecurityFinding::buffer_overflow(
                    format!(
                        "Extern function '{}' is unsafe and prone to buffer overflows",
                        func_name
                    ),
                    func.name.span,
                ));
            ...
            if matches!(func_name.as_str(), "system" | "exec" | "popen") {
                self.findings.push(SecurityFinding::injection(
                    format!(
                        "Extern function '{}' can execute commands - high injection risk",
                        func_name
                    ),
                    func.name.span,
                ));
        ...
        match stmt {
            Stmt::Let { name, value, .. } => {
                ...
                if let Expr::String(s) = &value.node {
                    ...
                    if (var_lower.contains("key")
                        || var_lower.contains("token")
                        || var_lower.contains("secret")
                        || var_lower.contains("password"))
                        && s.len() > 10
                    {
                        self.findings.push(SecurityFinding::hardcoded_secret(
                            format!(
                                "Suspicious string literal assigned to '{}': '{}'",
                                name.node,
                                self.truncate_string(s, 40)
                            ),
                            value.span,
                            Severity::High,
                        ));
        ...
        match expr {
            ...
            Expr::Binary { op, left, right } => {
                ...
                if matches!(op, BinOp::Add | BinOp::Mul | BinOp::Sub)
                    && (self.operates_on_user_input(&left.node)
                        || self.operates_on_user_input(&right.node))
                {
                    self.findings.push(SecurityFinding::integer_overflow(
                        "Arithmetic operation on potentially unchecked input may overflow"
                            .to_string(),
                        span,
                    ));
                }
        ...
        if let Expr::Ident(func_name) = &func.node {
            match func_name.as_str() {
                ...
                "load_byte" | "store_byte" | "load_i64" | "store_i64" => {
                    self.findings.push(SecurityFinding::buffer_overflow(
                        format!(
                            "Direct memory operation '{}' without bounds checking",
                            func_name
                        ),
                        span,
                    ));
                ...
                "memcpy" | "memmove" | "memset" => {
                    self.findings.push(SecurityFinding::buffer_overflow(
                        format!(
                            "Unsafe memory operation '{}' - verify buffer sizes",
                            func_name
                        ),
                        span,
                    ));
                ...
                "system" | "exec" | "execve" | "popen" => {
                    ...
                    self.findings.push(SecurityFinding::injection(
                        format!(
                            "Command execution function '{}' - validate and sanitize inputs",
                            func_name
                        ),
                        span,
                    ));
    ...
    fn contains_string_concat(&self, expr: &Expr) -> bool {
        match expr {
            Expr::Binary {
                ...
                self.is_string_expr(&left.node)
                    || self.is_string_expr(&right.node)
                    || self.contains_string_concat(&left.node)
                    || self.contains_string_concat(&right.node)
            }
            Expr::Call { func, .. } => {
