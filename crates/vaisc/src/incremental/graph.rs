        ...
        if let Some(deps) = self.forward_deps.get(file) {
            for dep in deps {
                if !state.index.contains_key(dep) {
                    ...
                    // SAFETY: low_link is always set before recursive return in Tarjan's algorithm
                    let dep_low = state.low_link.get(dep).copied().unwrap_or(0);
                    if let Some(file_low) = state.low_link.get_mut(file) {
                        *file_low = (*file_low).min(dep_low);
                    }
                } else if state.on_stack.contains(dep) {
                    // Back edge — index is always set for visited nodes in Tarjan's
                    let dep_index = state.index.get(dep).copied().unwrap_or(0);
                    if let Some(file_low) = state.low_link.get_mut(file) {
