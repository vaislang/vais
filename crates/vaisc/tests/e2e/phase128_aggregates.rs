...
impl Calc {
    fn compute(&self, a: i64, b: i64) -> i64 = self.base + a + b
}
fn main() -> i64 {
    c := Calc { base: 10 }
    c.compute(12, 20)
...
fn main() -> i64 = sides(Square) + sides(Triangle) + 35
"#,
        42,
    );
}
...
impl Box: Sizeable {
    fn size(&self) -> i64 = self.w * self.h
}
fn main() -> i64 {
    b := Box { w: 6, h: 7 }
    b.size()
...
struct Cube { side: i64 }
struct Flat { area: i64 }
impl Cube: Volume {
    fn vol(&self) -> i64 = self.side * self.side * self.side
}
impl Flat: Volume {
    fn vol(&self) -> i64 = self.area
}
fn main() -> i64 {
    f := Flat { area: 42 }
    f.vol()
