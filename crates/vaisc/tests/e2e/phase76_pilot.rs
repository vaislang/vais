...
fn e2e_p76_json_value_construction() {
    let source = r#"
fn strlen_ptr(s: i64) -> i64 {
    I load_byte(s) == 0 { 0 }
    else { 1 + strlen_ptr(s + 1) }
}
...
fn e2e_p76_dynamic_buffer() {
    let source = r#"
fn copy_bytes(dst: i64, src: i64, len: i64, idx: i64) -> i64 {
    I idx >= len { 0 } else {
...
fn buf_data(b: i64) -> i64 { load_i64(b) }
fn buf_len(b: i64) -> i64 { load_i64(b + 8) }
fn buf_cap(b: i64) -> i64 { load_i64(b + 16) }
...
fn buf_push_byte(b: i64, c: i64) -> i64 {
    buf_ensure(b, 1)
    data := buf_data(b)
    len := buf_len(b)
...
fn mem_zero(ptr: i64, count: i64, idx: i64) -> i64 {
    I idx >= count { 0 } else {
...
fn str_eq(a: i64, b: i64, len: i64) -> i64 {
...
fn obj_set(obj: i64, key: i64, key_len: i64, value: i64) -> i64 {
...
fn obj_get_rec(node: i64, key: i64, key_len: i64) -> i64 {
    I node == 0 { 0 }
    else {
        e_key := load_i64(node)
...
fn obj_get(obj: i64, key: i64, key_len: i64) -> i64 {
    buckets := load_i64(obj)
    cap := load_i64(obj + 16)
    h := hash_str(key, key_len) % cap
...
fn count_digits(n: i64, acc: i64) -> i64 {
    I n == 0 { acc }
    else { count_digits(n / 10, acc + 1) }
}
fn fill_digits(buf: i64, n: i64, pos: i64) -> i64 {
    I n == 0 { 0 }
    else {
        digit := n % 10
...
fn e2e_p76_dynamic_array() {
    let source = r#"
fn copy_arr_data(dst: i64, src: i64, len: i64, idx: i64) -> i64 {
    I idx >= len { 0 } else {
...
fn arr_push(a: i64, value: i64) -> i64 {
    len := load_i64(a + 8)
    cap := load_i64(a + 16)
    I len >= cap {
        new_cap := cap * 2
...
fn arr_len(a: i64) -> i64 { load_i64(a + 8) }
fn arr_get(a: i64, idx: i64) -> i64 { load_i64(load_i64(a) + idx * 8) }
...
fn e2e_p76_toml_key_quoting() {
    let source = r#"
fn needs_quoting(key: i64, len: i64) -> i64 {
    I len == 0 { 1 }
    else { needs_quoting_rec(key, len, 0) }
}
fn needs_quoting_rec(key: i64, len: i64, idx: i64) -> i64 {
    I idx >= len { 0 }
    else {
        c := load_byte(key + idx)
...
fn e2e_p76_entry_param_name_fix() {
    let source = r#"
fn process_entry(entry: i64, count: i64) -> i64 {
    I count <= 0 { entry }
    else { process_entry(entry + 1, count - 1) }
}
