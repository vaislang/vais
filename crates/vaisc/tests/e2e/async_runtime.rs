...
A fn add_ten(x: i64) -> i64 {
    x + 10
}

A fn pipeline(x: i64) -> i64 {
...
fn e2e_async_mixed_sync_async() {
    let source = r#"
fn sync_double(x: i64) -> i64 = x * 2

A fn async_add(a: i64, b: i64) -> i64 {
    a + b
}
...
fn main() -> i64 = add(mul(3, 4), mul(2, 3)) - 18
"#;
    assert_exit_code(source, 0);
}
...
fn e2e_http_str_eq_ignore_case() {
    ...
    let source = r#"
impl fn __str_eq_ignore_case(a: str, b: str) -> i64
fn main() -> i64 {
...
fn e2e_http_parse_url_port() {
    ...
    let source = r#"
impl fn __parse_url_port(url: str) -> i64
fn main() -> i64 {
    port := __parse_url_port("http://example.com:3000/api")
    I port == 3000 { 0 } else { 1 }
}
"#;
    let result = compile_and_run_with_extra_sources(source, &[&rt]).unwrap();
    assert_eq!(
...
fn e2e_http_parse_url_host() {
    ...
    let source = r#"
impl fn __parse_url_host(url: str) -> str
impl fn __str_eq(a: str, b: str) -> i64
fn main() -> i64 {
    host := __parse_url_host("http://localhost:8080/path")
    I __str_eq(host, "localhost") == 1 { 0 } else { 1 }
}
"#;
    let result = compile_and_run_with_extra_sources(source, &[&rt]).unwrap();
    assert_eq!(
...
fn e2e_http_parse_url_path() {
    ...
    let source = r#"
impl fn __parse_url_path(url: str) -> str
impl fn __str_eq(a: str, b: str) -> i64
fn main() -> i64 {
    path := __parse_url_path("http://example.com:8080/api/users")
    I __str_eq(path, "/api/users") == 1 { 0 } else { 1 }
}
"#;
    let result = compile_and_run_with_extra_sources(source, &[&rt]).unwrap();
    assert_eq!(
...
impl fn __str_copy_to(dst: i64, src: str) -> i64
...
impl fn __malloc(size: i64) -> i64
impl fn __free(ptr: i64) -> i64
impl fn __i64_to_str(dst: i64, value: i64) -> i64
fn main() -> i64 {
    buf := __malloc(32)
    written := __i64_to_str(buf, 12345)
    __free(buf)
    I written == 5 { 0 } else { 1 }
}
"#;
    let result = compile_and_run_with_extra_sources(source, &[&rt]).unwrap();
    assert_eq!(
