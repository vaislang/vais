...
fn str_find_first_non_ws(data: i64, len: i64, idx: i64) -> i64 {
    I idx >= len { len }
    else {
...
fn str_find_last_non_ws(data: i64, len: i64) -> i64 {
    I len <= 0 { 0 }
    else {
...
fn str_trim(data: i64, len: i64) -> i64 {
...
fn e2e_p74_str_to_upper() {
    let source = r#"
fn str_to_upper(data: i64, len: i64) -> i64 {
    buf := malloc(len + 1)
    str_to_upper_rec(data, len, 0, buf)
    store_byte(buf + len, 0)
    buf
}
fn str_to_upper_rec(data: i64, len: i64, idx: i64, buf: i64) -> i64 {
    I idx >= len { 0 }
    else {
        c := load_byte(data + idx)
        out_c := I c >= 97 && c <= 122 { c - 32 } else { c }
...
fn e2e_p74_str_to_lower() {
    let source = r#"
fn str_to_lower(data: i64, len: i64) -> i64 {
    buf := malloc(len + 1)
    str_to_lower_rec(data, len, 0, buf)
    store_byte(buf + len, 0)
    buf
}
fn str_to_lower_rec(data: i64, len: i64, idx: i64, buf: i64) -> i64 {
    I idx >= len { 0 }
    else {
        c := load_byte(data + idx)
        out_c := I c >= 65 && c <= 90 { c + 32 } else { c }
        store_byte(buf + idx, out_c)
...
fn e2e_p74_str_starts_with() {
    let source = r#"
fn str_starts_with_rec(data: i64, prefix: i64, prefix_len: i64, idx: i64) -> i64 {
    I idx >= prefix_len { 1 }
    else I load_byte(data + idx) != load_byte(prefix + idx) { 0 }
    else { str_starts_with_rec(data, prefix, prefix_len, idx + 1) }
}
fn str_starts_with(data: i64, data_len: i64, prefix: i64, prefix_len: i64) -> i64 {
    I prefix_len > data_len { return 0 }
...
fn e2e_p74_str_ends_with() {
    let source = r#"
fn str_ends_with_rec(data: i64, offset: i64, suffix: i64, suffix_len: i64, idx: i64) -> i64 {
    I idx >= suffix_len { 1 }
    else I load_byte(data + offset + idx) != load_byte(suffix + idx) { 0 }
    else { str_ends_with_rec(data, offset, suffix, suffix_len, idx + 1) }
}
fn str_ends_with(data: i64, data_len: i64, suffix: i64, suffix_len: i64) -> i64 {
    I suffix_len > data_len { return 0 }
...
fn e2e_p74_str_index_of() {
    let source = r#"
fn str_match_at(haystack: i64, offset: i64, needle: i64, n_len: i64, idx: i64) -> i64 {
    I idx >= n_len { 1 }
    else I load_byte(haystack + offset + idx) != load_byte(needle + idx) { 0 }
    else { str_match_at(haystack, offset, needle, n_len, idx + 1) }
}
fn str_index_of_rec(haystack: i64, h_len: i64, needle: i64, n_len: i64, pos: i64) -> i64 {
    I pos + n_len > h_len { 0 - 1 }
    else {
        I str_match_at(haystack, pos, needle, n_len, 0) == 1 { pos }
        else { str_index_of_rec(haystack, h_len, needle, n_len, pos + 1) }
    }
}
fn str_index_of(haystack: i64, h_len: i64, needle: i64, n_len: i64) -> i64 {
    I n_len == 0 { return 0 }
    I n_len > h_len { return 0 - 1 }
...
fn str_replace_rec(data: i64, d_len: i64, old: i64, o_len: i64, new_s: i64, n_len: i64, pos: i64, buf: i64, out_pos: i64) -> i64 {
    I pos >= d_len { out_pos }
    else I pos + o_len <= d_len && str_match_at(data, pos, old, o_len, 0) == 1 {
...
fn str_replace(data: i64, d_len: i64, old: i64, o_len: i64, new_s: i64, n_len: i64) -> i64 {
...
fn e2e_p74_str_split_and_count() {
    let source = r#"
fn str_count_delim(data: i64, len: i64, delim: i64, idx: i64, count: i64) -> i64 {
    I idx >= len { count }
    else {
        c := load_byte(data + idx)
        I c == delim { str_count_delim(data, len, delim, idx + 1, count + 1) }
        else { str_count_delim(data, len, delim, idx + 1, count) }
    }
}
fn main() -> i64 {
...
fn e2e_p74_str_from_int() {
    let source = r#"
fn str_from_int_digits(tmp: i64, val: i64, count: i64) -> i64 {
    I val == 0 { count }
    else {
...
fn str_from_int_rev_rec(buf: i64, tmp: i64, idx: i64, pos: i64) -> i64 {
    I idx < 0 { pos }
    else {
...
fn toml_strlen_ptr(s: i64) -> i64 { toml_strlen_rec(s, 0) }
...
fn toml_peek(p: i64) -> i64 {
    pos := load_i64(p + 8)
    len := load_i64(p + 16)
    I pos >= len { 0 } else { load_byte(load_i64(p) + pos) }
}
fn toml_advance(p: i64) -> i64 {
    pos := load_i64(p + 8)
    len := load_i64(p + 16)
...
fn parse_bare_key_len(p: i64, len: i64) -> i64 {
    c := toml_peek(p)
...
fn toml_parse_digits(p: i64, acc: i64) -> i64 {
    c := toml_peek(p)
    I c == 95 { toml_advance(p); toml_parse_digits(p, acc) }
    else I c >= 48 && c <= 57 { toml_advance(p); toml_parse_digits(p, acc * 10 + (c - 48)) }
    else { acc }
}
fn main() -> i64 {
    # Parse "1_000" -> 1000
...
fn e2e_p74_yaml_parse_integer() {
    let source = r#"
fn yaml_is_digit(c: i64) -> i64 {
    I c >= 48 && c <= 57 { 1 } else { 0 }
}
fn yaml_parse_buf_digits(buf: i64, idx: i64, len: i64, acc: i64) -> i64 {
    I idx >= len { acc }
    else {
        c := load_byte(buf + idx)
...
fn e2e_p74_yaml_count_indent() {
    let source = r#"
fn count_indent(data: i64, pos: i64, count: i64) -> i64 {
...
fn e2e_p74_yaml_rtrim() {
    let source = r#"
fn yaml_rtrim(buf: i64, len: i64) -> i64 {
    I len <= 0 { 0 }
    else {
...
fn str_eq_rec(a: i64, b: i64, len: i64, idx: i64) -> i64 {
    I idx >= len { 1 }
    else I load_byte(a + idx) != load_byte(b + idx) { 0 }
    else { str_eq_rec(a, b, len, idx + 1) }
}
fn init_mem(ptr: i64, count: i64, idx: i64) -> i64 {
    I idx >= count { 0 } else {
...
fn table_set(tbl: i64, key: i64, key_len: i64, value: i64) -> i64 {
...
fn table_get_rec(node: i64, key: i64, key_len: i64) -> i64 {
    I node == 0 { 0 } else {
        nk := load_i64(node)
        nkl := load_i64(node + 8)
...
fn table_get(tbl: i64, key: i64, key_len: i64) -> i64 {
    buckets := load_i64(tbl)
    cap := load_i64(tbl + 16)
    h := hash_str(key, key_len) % cap
...
fn e2e_p74_str_split_fill_basic() {
    let source = r#"
fn str_split_fill(data: i64, len: i64, delim: i64, idx: i64, start: i64, part_idx: i64, parts: i64) -> i64 {
    I idx >= len {
        part_len := idx - start
        ...
        0
    } else I load_byte(data + idx) == delim {
        part_len := idx - start
        part_data := malloc(part_len + 16)
        I part_len > 0 { memcpy(part_data, data + start, part_len) }
        store_byte(part_data + part_len, 0)
...
fn str_contains(haystack: i64, h_len: i64, needle: i64, n_len: i64) -> i64 {
    I n_len == 0 { return 1 }
    I n_len > h_len { return 0 }
...
fn main() -> i64 {
    ...
    is_array := I c0 == 91 && c8 == 93 { 1 } else { 0 }
...
fn main() -> i64 {
    ...
    result_n := I n_char == 110 { 10 } else { n_char }
    result_t := I t_char == 116 { 9 } else { t_char }
    result_bs := I backslash == 92 { 92 } else { backslash }
