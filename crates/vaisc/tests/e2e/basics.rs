...
#[test]
fn e2e_addition() {
    assert_exit_code("fn main()->i64 = 30 + 12", 42);
}
...
#[test]
fn e2e_modulo() {
    assert_exit_code("fn main()->i64 = 47 % 5", 2);
}

#[test]
fn e2e_nested_arithmetic() {
    assert_exit_code("fn main()->i64 = (3 + 4) * (2 + 4)", 42);
}
...
fn e2e_comparison_true() {
    // true = 1
    assert_exit_code("fn main()->i64 = I 10 > 5 { 1 } else { 0 }", 1);
}

#[test]
fn e2e_comparison_false() {
    assert_exit_code("fn main()->i64 = I 5 > 10 { 1 } else { 0 }", 0);
}

#[test]
fn e2e_equality() {
    assert_exit_code("fn main()->i64 = I 42 == 42 { 1 } else { 0 }", 1);
}

#[test]
fn e2e_inequality() {
    assert_exit_code("fn main()->i64 = I 42 != 43 { 1 } else { 0 }", 1);
}
...
fn e2e_sum_to_n() {
    let source = r#"
fn sum(n: i64) -> i64 = I n == 0 { 0 } else { n + @(n-1) }
fn main() -> i64 = sum(9)
"#;
...
fn e2e_power() {
    let source = r#"
fn pow(base: i64, exp: i64) -> i64 = I exp == 0 { 1 } else { base * @(base, exp - 1) }
fn main() -> i64 = pow(2, 5)
"#;
    // 2^5 = 32
...
fn e2e_error_undefined_function() {
    assert_compile_error("fn main()->i64 = unknown_func(42)");
}
...
fn e2e_mutual_recursion() {
    let source = r#"
fn is_even(n: i64) -> i64 = I n == 0 { 1 } else { is_odd(n - 1) }
fn is_odd(n: i64) -> i64 = I n == 0 { 0 } else { is_even(n - 1) }
...
fn e2e_bitwise_and() {
    // 0xFF & 0x0F = 0x0F = 15
    assert_exit_code("fn main()->i64 = 255 & 15", 15);
}

#[test]
fn e2e_bitwise_or() {
    // 0x20 | 0x0A = 0x2A = 42
    assert_exit_code("fn main()->i64 = 32 | 10", 42);
}

#[test]
fn e2e_bitwise_xor() {
    // 0xFF ^ 0xD5 = 0x2A = 42
    assert_exit_code("fn main()->i64 = 255 ^ 213", 42);
}
...
fn e2e_bitwise_right_shift() {
    // 168 >> 2 = 42
    assert_exit_code("fn main()->i64 = 168 >> 2", 42);
}
...
fn e2e_logical_and_true() {
    let source = "fn main()->i64 = I 1 > 0 && 2 > 1 { 42 } else { 0 }";
    assert_exit_code(source, 42);
}

#[test]
fn e2e_logical_and_false() {
    let source = "fn main()->i64 = I 1 > 0 && 2 < 1 { 0 } else { 42 }";
    assert_exit_code(source, 42);
}

#[test]
fn e2e_logical_or_true() {
    let source = "fn main()->i64 = I 0 > 1 || 2 > 1 { 42 } else { 0 }";
    assert_exit_code(source, 42);
}

#[test]
fn e2e_logical_or_both_false() {
    let source = "fn main()->i64 = I 0 > 1 || 0 > 2 { 0 } else { 42 }";
    assert_exit_code(source, 42);
}
...
fn e2e_recursive_count_to_n() {
    ...
    let source = r#"
fn count(n: i64) -> i64 = I n == 0 { 0 } else { 1 + @(n - 1) }
fn main() -> i64 = count(10)
"#;
...
fn e2e_recursive_sum_to_n() {
    ...
    let source = r#"
fn sum_acc(n: i64, acc: i64) -> i64 = I n == 0 { acc } else { @(n - 1, acc + n) }
fn main() -> i64 = sum_acc(9, 0)
"#;
    // 9+8+7+6+5+4+3+2+1 = 45
    assert_exit_code(source, 45);
}
...
fn e2e_recursive_product() {
    ...
    let source = r#"
fn repeat_mul(base: i64, times: i64, acc: i64) -> i64 =
    I times == 0 { acc } else { @(base, times - 1, acc * base) }
fn main() -> i64 = repeat_mul(2, 5, 1)
"#;
    assert_exit_code(source, 32);
}
...
fn e2e_recursive_countdown_sum() {
    ...
    let source = r#"
fn countdown_sum(n: i64) -> i64 = I n == 0 { 0 } else { n + @(n - 1) }
fn main() -> i64 = countdown_sum(10)
"#;
    assert_exit_code(source, 55);
}
...
fn e2e_recursive_multiply_add() {
    ...
    let source = r#"
fn mul_add(base: i64, times: i64, acc: i64) -> i64 =
    I times == 0 { acc } else { @(base, times - 1, acc + base) }
fn main() -> i64 = mul_add(5, 5, 0)
"#;
    // 5 * 5 = 25
...
fn e2e_tower_of_hanoi_count() {
    ...
    // hanoi(6) = 63
    let source = r#"
fn hanoi(n: i64) -> i64 = I n == 0 { 0 } else { 2 * @(n - 1) + 1 }
fn main() -> i64 = hanoi(6)
"#;
...
fn e2e_triple_mutual_recursion() {
    ...
    let source = r#"
fn fa(n: i64) -> i64 = I n == 0 { 1 } else { fb(n - 1) }
fn fb(n: i64) -> i64 = I n == 0 { 2 } else { fc(n - 1) }
fn fc(n: i64) -> i64 = I n == 0 { 3 } else { fa(n - 1) }
fn main() -> i64 = fa(7)
"#;
...
fn e2e_cascading_conditional_calls() {
    let source = r#"
fn pick(a: i64, b: i64) -> i64 = I a > b { a } else { b }
fn bump(x: i64) -> i64 = x + 1
fn main() -> i64 = bump(pick(20, bump(20)))
"#;
...
fn e2e_zero_division_guard() {
    let source = r#"
fn safe_div(a: i64, b: i64) -> i64 = I b == 0 { 0 } else { a / b }
fn main() -> i64 = safe_div(100, 0)
"#;
    assert_exit_code(source, 0);
}
...
fn e2e_deeply_nested_calls() {
    let source = r#"
fn f1(x: i64) -> i64 = x + 1
fn f2(x: i64) -> i64 = f1(x) + 1
fn f3(x: i64) -> i64 = f2(x) + 1
fn f4(x: i64) -> i64 = f3(x) + 1
fn f5(x: i64) -> i64 = f4(x) + 1
fn f6(x: i64) -> i64 = f5(x) + 1
fn f7(x: i64) -> i64 = f6(x) + 1
fn f8(x: i64) -> i64 = f7(x) + 1
fn f9(x: i64) -> i64 = f8(x) + 1
fn f10(x: i64) -> i64 = f9(x) + 1
fn main() -> i64 = f10(32)
"#;
    // 32 + 10 = 42
    assert_exit_code(source, 42);
...
fn e2e_boolean_as_integer() {
    ...
    let source = r#"
fn btoi(cond: i64) -> i64 = I cond > 0 { 1 } else { 0 }
fn main() -> i64 {
    a := btoi(10)
    b := btoi(0)
    a + b
...
fn e2e_struct_computed_fields() {
    let source = r#"
struct Result { value: i64, ok: i64 }
fn main() -> i64 {
    r := Result { value: 6 * 7, ok: 1 }
    I r.ok == 1 { r.value } else { 0 }
}
"#;
    assert_exit_code(source, 42);
}
