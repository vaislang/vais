...
#[test]
fn e2e_std_http_client_loopback_malformed_status_parse_error_runtime_smoke() {
    let temp = tempfile::TempDir::new().expect("temp dir");
    let main_path = temp.path().join("main.vais");
    let exe_path = temp.path().join("http_client_malformed_status_smoke");
...
fn main() -> i64 {
    response := http_get("http://127.0.0.1:__PORT__/malformed")
    I response.error_code != -6 { return 1 }
    I response.status != 0 { return 2 }
    I response.is_ok() != 0 { return 3 }
    response.drop()
    0
}
"#
    .replace("__PORT__", &port.to_string());
    std::fs::write(&main_path, source).expect("write http_client malformed status fixture");
    ...
    assert!(
        build.status.success(),
        "std/http_client malformed status fixture failed to build\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );

    let server = thread::spawn(move || accept_malformed_status_response(listener));
    let run = Command::new(&exe_path)
        .current_dir(temp.path())
        .output()
        .expect("run std/http_client malformed status fixture");
    let request_text = server
        .join()
        .expect("join loopback malformed status server");

    assert_eq!(
        run.status.code().unwrap_or(-1),
        0,
        "std/http_client malformed status fixture exited unexpectedly\nstdout:\n{}\nstderr:\n{}\nrequest:\n{}",
        String::from_utf8_lossy(&run.stdout),
        String::from_utf8_lossy(&run.stderr),
        request_text
    );
    assert!(
        request_text.contains("GET /malformed HTTP/1.1"),
        "request did not contain expected request line:\n{}",
        request_text
    );
    assert!(
...
#[test]
fn e2e_std_http_client_loopback_malformed_content_length_parse_error_runtime_smoke() {
    let temp = tempfile::TempDir::new().expect("temp dir");
    let main_path = temp.path().join("main.vais");
    let exe_path = temp
        .path()
        .join("http_client_malformed_content_length_smoke");
...
fn main() -> i64 {
    response := http_get("http://127.0.0.1:__PORT__/bad-length")
    I response.error_code != -6 { return 1 }
    I response.status != 0 { return 2 }
    I response.body_len != 0 { return 3 }
    I response.is_ok() != 0 { return 4 }
...
"#
    .replace("__PORT__", &port.to_string());
    std::fs::write(&main_path, source).expect("write http_client malformed content-length fixture");
    ...
    assert!(
        build.status.success(),
        "std/http_client malformed content-length fixture failed to build\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );

    let server = thread::spawn(move || accept_malformed_content_length_response(listener));
    let run = Command::new(&exe_path)
        .current_dir(temp.path())
        .output()
        .expect("run std/http_client malformed content-length fixture");
    let request_text = server
        .join()
        .expect("join loopback malformed content-length server");

    assert_eq!(
        run.status.code().unwrap_or(-1),
        0,
        "std/http_client malformed content-length fixture exited unexpectedly\nstdout:\n{}\nstderr:\n{}\nrequest:\n{}",
        String::from_utf8_lossy(&run.stdout),
        String::from_utf8_lossy(&run.stderr),
        request_text
    );
    assert!(
        request_text.contains("GET /bad-length HTTP/1.1"),
        "request did not contain expected request line:\n{}",
        request_text
    );
    assert!(
...
#[test]
fn e2e_std_http_client_loopback_malformed_header_line_parse_error_runtime_smoke() {
    let temp = tempfile::TempDir::new().expect("temp dir");
    let main_path = temp.path().join("main.vais");
    let exe_path = temp.path().join("http_client_malformed_header_smoke");
...
fn main() -> i64 {
    response := http_get("http://127.0.0.1:__PORT__/bad-header")
    I response.error_code != -6 { return 1 }
    I response.status != 0 { return 2 }
    I response.body_len != 0 { return 3 }
    I response.is_ok() != 0 { return 4 }
...
"#
    .replace("__PORT__", &port.to_string());
    std::fs::write(&main_path, source).expect("write http_client malformed header fixture");
    ...
    assert!(
        build.status.success(),
        "std/http_client malformed header fixture failed to build\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );

    let server = thread::spawn(move || accept_malformed_header_line_response(listener));
    let run = Command::new(&exe_path)
        .current_dir(temp.path())
        .output()
        .expect("run std/http_client malformed header fixture");
    let request_text = server
        .join()
        .expect("join loopback malformed header server");

    assert_eq!(
        run.status.code().unwrap_or(-1),
        0,
        "std/http_client malformed header fixture exited unexpectedly\nstdout:\n{}\nstderr:\n{}\nrequest:\n{}",
        String::from_utf8_lossy(&run.stdout),
        String::from_utf8_lossy(&run.stderr),
        request_text
    );
    assert!(
        request_text.contains("GET /bad-header HTTP/1.1"),
        "request did not contain expected request line:\n{}",
        request_text
    );
    assert!(
...
#[test]
fn e2e_std_http_client_loopback_chunked_response_runtime_smoke() {
    let temp = tempfile::TempDir::new().expect("temp dir");
    let main_path = temp.path().join("main.vais");
    let exe_path = temp.path().join("http_client_chunked_response_smoke");
...
fn main() -> i64 {
    response := http_get("http://127.0.0.1:__PORT__/chunked")
    I response.error_code != 0 { return 1 }
    I response.status != 200 { return 2 }
    I response.body_len != 11 { return 3 }
    body := response.body_text()
    I smoke_contains(body, "hello world") != 1 { return 4 }
    response.drop()
    0
}
"#
    .replace("__PORT__", &port.to_string());
    std::fs::write(&main_path, source).expect("write http_client chunked fixture");
    ...
    assert!(
        build.status.success(),
        "std/http_client chunked fixture failed to build\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );

    let server = thread::spawn(move || accept_chunked_response(listener));
    let run = Command::new(&exe_path)
        .current_dir(temp.path())
        .output()
        .expect("run std/http_client chunked fixture");
    let request_text = server.join().expect("join loopback chunked server");

    assert_eq!(
        run.status.code().unwrap_or(-1),
        0,
        "std/http_client chunked fixture exited unexpectedly\nstdout:\n{}\nstderr:\n{}\nrequest:\n{}",
        String::from_utf8_lossy(&run.stdout),
        String::from_utf8_lossy(&run.stderr),
        request_text
    );
    assert!(
        request_text.contains("GET /chunked HTTP/1.1"),
        "request did not contain expected request line:\n{}",
        request_text
    );
    assert!(
...
fn e2e_std_http_client_loopback_keep_alive_stale_get_retries_runtime_smoke() {
    let temp = tempfile::TempDir::new().expect("temp dir");
    let main_path = temp.path().join("main.vais");
    let exe_path = temp.path().join("http_client_stale_keep_alive_smoke");
...
fn main() -> i64 {
    ...
    first := client.get("http://127.0.0.1:__PORT__/first")
    I first.error_code != 0 { return 1 }
    I first.status != 200 { return 2 }
    first_body := first.body_text()
    I smoke_contains(first_body, "first-ok") != 1 { return 3 }
    first.drop()

    second := client.get("http://127.0.0.1:__PORT__/second")
    I second.error_code != 0 { return 4 }
    I second.status != 200 { return 5 }
    second_body := second.body_text()
    I smoke_contains(second_body, "second-ok") != 1 { return 6 }
    second.drop()
    client.close()
    0
}
"#
    .replace("__PORT__", &port.to_string());
    std::fs::write(&main_path, source).expect("write http_client stale keep-alive fixture");
    ...
    assert!(
        build.status.success(),
        "std/http_client stale keep-alive fixture failed to build\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );

    let server = thread::spawn(move || accept_stale_keep_alive_then_retry(listener));
    let run = Command::new(&exe_path)
        .current_dir(temp.path())
        .output()
        .expect("run std/http_client stale keep-alive fixture");
    let requests = server
        .join()
        .expect("join loopback stale keep-alive server");

    assert_eq!(
        run.status.code().unwrap_or(-1),
        0,
        "std/http_client stale keep-alive fixture exited unexpectedly\nstdout:\n{}\nstderr:\n{}\nfirst request:\n{}\nsecond request:\n{}",
        String::from_utf8_lossy(&run.stdout),
        String::from_utf8_lossy(&run.stderr),
        requests.0,
        requests.1
    ...
    assert!(
        requests.0.contains("GET /first HTTP/1.1"),
        "first request did not contain expected request line:\n{}",
        requests.0
    );
    assert!(
        requests.1.contains("GET /second HTTP/1.1"),
        "second request did not arrive on a fresh retry connection:\n{}",
        requests.1
    );
    assert!(
        requests.0.contains("Host: 127.0.0.1:") && requests.1.contains("Host: 127.0.0.1:"),
        "stale keep-alive requests did not contain Host headers with loopback port:\nfirst:\n{}\nsecond:\n{}",
        requests.0,
        requests.1
    );
}
...
fn accept_malformed_status_response(listener: TcpListener) -> String {
    let (mut stream, request) = accept_http_request(&listener);
    let response = "NOTHTTP 200 OK\r\nContent-Length: 5\r\nConnection: close\r\n\r\nhello";
    stream
        .write_all(response.as_bytes())
        .expect("write std/http_client malformed status response");
    stream
        .flush()
...
fn accept_malformed_content_length_response(listener: TcpListener) -> String {
    let (mut stream, request) = accept_http_request(&listener);
    let response = "HTTP/1.1 200 OK\r\nContent-Length: nope\r\nConnection: close\r\n\r\n";
    stream
        .write_all(response.as_bytes())
        .expect("write std/http_client malformed content-length response");
    stream
        .flush()
        .expect("flush std/http_client malformed content-length response");

    request
}

fn accept_malformed_header_line_response(listener: TcpListener) -> String {
    let (mut stream, request) = accept_http_request(&listener);
    let response = "HTTP/1.1 200 OK\r\nContent-Type text/plain\r\nConnection: close\r\n\r\nhello";
    stream
        .write_all(response.as_bytes())
        .expect("write std/http_client malformed header response");
    stream
        .flush()
        .expect("flush std/http_client malformed header response");

    request
}

fn accept_chunked_response(listener: TcpListener) -> String {
    let (mut stream, request) = accept_http_request(&listener);
    let headers = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nTransfer-Encoding: chunked\r\nConnection: close\r\n\r\n";
    stream
        .write_all(headers.as_bytes())
        .expect("write std/http_client chunked headers");
    stream
        .flush()
        .expect("flush std/http_client chunked headers");
    thread::sleep(Duration::from_millis(50));
    let chunks = "5\r\nhello\r\n1\r\n \r\n5\r\nworld\r\n0\r\n\r\n";
    stream
        .write_all(chunks.as_bytes())
        .expect("write std/http_client chunked body");
    stream.flush().expect("flush std/http_client chunked body");
...
fn accept_stale_keep_alive_then_retry(listener: TcpListener) -> (String, String) {
    let (mut first_stream, first_request) = accept_http_request(&listener);
    let first_body = "first-ok";
    ...
    first_stream
        ...
        .expect("write std/http_client stale keep-alive response");
    first_stream
        .flush()
        .expect("flush std/http_client stale keep-alive response");
    drop(first_stream);

    let (mut second_stream, second_request) = accept_http_request(&listener);
    let second_body = "second-ok";
    ...
    second_stream
        ...
        .expect("write std/http_client stale keep-alive retry response");
    second_stream
        .flush()
        .expect("flush std/http_client stale keep-alive retry response");
