...
impl Point {
    fn distance_sq(&self) -> i64 = self.x * self.x + self.y * self.y
}
struct Line {
    start_x: i64,
    start_y: i64,
...
fn test_param_type_infer_multi_param() {
    let source = r#"
fn clamp(val, lo, hi) -> i64 {
    I val < lo { return lo }
    I val > hi { return hi }
    return val
}
fn main() -> i64 {
    I clamp(15, 0, 10) == 10 { return 0 }
    return 1
}
"#;
    assert_exit_code(source, 0);
...
fn test_infer_mixed_explicit_inferred() {
    let source = r#"
fn clamp_max(x: i64, limit) -> i64 {
    I x > limit { return limit }
    return x
}
fn main() -> i64 {
    return clamp_max(50, 42)
}
"#;
    assert_exit_code(source, 42);
}
