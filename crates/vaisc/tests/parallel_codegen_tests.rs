...
fn test_single_module_codegen() {
    ...
    let main_file = create_vais_file(temp_dir.path(), "main.vais", "fn main() -> i32 { R 42 }");
...
fn test_dependency_graph_levels() {
    ...
    let base_content = "fn base_func() -> i32 { R 1 }";
    let util_content = "fn util_func() -> i32 { R base_func() + 1 }";
    let main_content = "fn main() -> i32 { R util_func() + 1 }";
...
fn test_independent_modules_parallel() {
    ...
    let module_a_content = "fn func_a() -> i32 { R 1 }";
    let module_b_content = "fn func_b() -> i32 { R 2 }";
