...
fn windows_path_backslash_escaping() {
    ...
    let source = r#"
fn get_windows_path() -> i64 {
    path := "C:\\Users\\Admin\\file.txt"
    # Backslashes must be escaped in Vais strings
    0
}
fn main() -> i64 = get_windows_path()
"#;
...
fn main() -> i64 = check_user_profile()
"#;
...
fn mock_platform() -> i64 = PLATFORM_WINDOWS
...
impl Reactor {
    fn new() -> Reactor {
        ...
        backend_fd := I platform == PLATFORM_WINDOWS {
            1
        } else I platform == PLATFORM_LINUX {
            2
        } else {
            3
        }
    ...
    fn is_windows(&self) -> i64 {
        I self.platform == PLATFORM_WINDOWS { 1 } else { 0 }
    }
}
...
impl Socket {
    fn new(family: i64, sock_type: i64) -> Socket {
        Socket { fd: 0, family: family, sock_type: sock_type }
    }

    fn is_tcp(&self) -> i64 {
        I self.sock_type == SOCK_STREAM { 1 } else { 0 }
    }
}
...
impl Thread {
    fn spawn(func_id: i64, arg: i64) -> Thread {
...
fn tls_load_cert_mock() -> i64 {
    # Simulate loading from "C:\\certs\\server.crt"
    0
}

fn tls_load_key_mock() -> i64 {
...
fn log_open_mock(append: i64) -> i64 {
    # Simulate opening "C:\\logs\\app.log"
    0
}

fn log_set_level_mock(level: i64) -> i64 {
    0
}
...
impl LogConfig {
    ...
    fn set_level(&self, level: i64) -> LogConfig {
        LogConfig {
            level: level,
...
fn compress_file_mock(algorithm: i64, level: i64) -> i64 {
...
impl CompressConfig {
    ...
    fn with_algorithm(&self, algo: i64) -> CompressConfig {
        CompressConfig {
            algorithm: algo,
            level: self.level
        }
...
fn mock_get_platform() -> i64 = PLATFORM_MACOS

fn create_directory_mock(platform: i64) -> i64 {
    0
}
...
fn mock_reg_open(root: i64, access: i64) -> i64 {
    I root == 0 { 1 } else { 0 }
}
...
impl RegistryKey {
    fn open(root: i64, access: i64) -> RegistryKey {
...
# IOCP event buffer
struct IOCPEventBuffer {
    events: i64,
    capacity: i64,
    count: i64
}

impl IOCPEventBuffer {
    fn new(capacity: i64) -> IOCPEventBuffer {
    ...
    fn get_event(&self, index: i64) -> i64 {
        I index >= self.count {
            0
        } else {
