...
fn test_public_function_generates_export() {
    ...
    assert!(js.contains("export function greet()"), "Public function should have 'export'");
}
...
fn test_private_function_no_export() {
    ...
    assert!(!js.contains("export function helper"), "Private function should not export");
}
...
fn test_function_with_default_params() {
    ...
    assert!(js.contains("name = \"world\""), "Should have default parameter value");
}
...
fn test_struct_generates_class() {
    ...
    assert!(js.contains("constructor(x, y)"), "Should have constructor with fields");
...
fn test_public_struct_exports() {
    ...
    assert!(js.contains("export class Point"), "Public struct should export");
}
...
fn test_struct_single_field() {
    ...
    assert!(js.contains("class Wrapper"), "Single field struct should work");
...
fn test_enum_generates_frozen_object() {
    ...
    assert!(js.contains("const Color = Object.freeze"), "Enum should be frozen object");
...
fn test_enum_tuple_variant() {
    ...
    assert!(js.contains("Circle(__0)"), "Tuple variant should be factory function");
    assert!(js.contains("Rect(__0, __1)"), "Multi-arg tuple variant should work");
...
fn test_enum_public_export() {
    ...
    assert!(js.contains("export const Direction"), "Public enum should export");
}
...
fn test_impl_instance_method() {
    let js = parse_and_generate(r#"
        S Counter { value: i64 }
        X Counter {
            F increment(self) { self.value + 1 }
        }
    "#);
    assert!(js.contains("Counter.prototype.increment"), "Instance method on prototype");
}
...
fn test_impl_static_method() {
    ...
    assert!(!js.contains("Counter.prototype.new_counter"), "Should NOT be on prototype");
}
...
fn test_impl_trait() {
    ...
    "#);
    assert!(js.contains("__implements"), "Trait impl should track __implements");
...
fn test_trait_generates_class() {
    ...
    assert!(js.contains("throw new Error"), "Abstract method should throw");
}
...
fn test_const_declaration() {
    ...
    assert!(js.contains("const MAX_SIZE = 100"), "Const should generate const");
}
...
fn test_use_generates_import() {
    ...
    assert!(js.contains("import * as math from './math.js'"), "Use should become import");
}
...
fn test_function_with_server_attribute() {
    ...
    "#);
    assert!(js.contains("async function loadItems()"), "Server function should generate");
}
...
fn test_function_with_wasm_attribute() {
    ...
    "#);
    assert!(js.contains("function processData(raw)"), "Wasm function should generate");
}
...
fn test_combined_vaisx_script_output() {
    ...
    assert!(js.contains("class __VxProps__"), "Props struct should become class");
    assert!(js.contains("function increment(count)"), "increment should generate");
    assert!(js.contains("function reset()"), "reset should generate");
    assert!(js.contains("export function formatCount(n)"), "Public fn should export");
}
...
fn test_async_server_function_output() {
    ...
    assert!(js.contains("async function loadItems()"), "Server load should be async");
    assert!(js.contains("async function saveItem(item)"), "Server save should be async");
}
...
fn test_source_map_creation() {
    ...
    assert!(json.contains("component.vaisx"), "Should reference source file");
    assert!(json.contains("component.js"), "Should reference generated file");

    let inline = map.to_inline_comment();
    assert!(inline.starts_with("//# sourceMappingURL=data:application/json;charset=utf-8;base64,"),
...
fn test_tree_shaking_removes_unused() {
    ...
    let module = vais_parser::parse(r#"
        P F used() -> i64 { 1 }
        F unused() -> i64 { 2 }
    "#).expect("Parse failed");
    ...
    let has_used = shaken.items.iter().any(|item| {
        matches!(&item.node, vais_ast::Item::Function(f) if f.name.node == "used")
    });
    assert!(has_used, "Public function should be kept");

    // Private unreferenced function should be removed
    let has_unused = shaken.items.iter().any(|item| {
        matches!(&item.node, vais_ast::Item::Function(f) if f.name.node == "unused")
    });
