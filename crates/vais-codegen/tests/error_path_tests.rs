...
fn test_undefined_var_simple() {
    let err = gen_err("fn test()->i64{return x}");
    assert!(
        err.contains("x"),
...
fn test_undefined_var_in_binary_op() {
    let err = gen_err("fn test()->i64{return a+b}");
...
fn test_type_error_indexing_non_array() {
    let err = gen_err("fn test()->i64{x:=42;return x[0]}");
    assert!(
...
fn test_field_access_non_struct() {
    let err = gen_err("fn test()->i64{x:=42;return x.y}");
    assert!(
        !err.is_empty(),
...
fn test_struct_field_not_found() {
    let err = gen_err("struct Point{x:i64,y:i64} fn test(p:Point)->i64=p.z");
    assert!(err.contains("z") || err.contains("field"), "Error: {}", err);
}

#[test]
fn test_struct_field_suggestion() {
    let err = gen_err("struct Point{x:i64,y:i64} fn test(p:Point)->i64=p.X");
...
fn test_valid_variable_binding() {
    let ir = gen_ok("fn test()->i64{x:=42;return x}");
    assert!(ir.contains("define i64 @test"));
}

#[test]
fn test_valid_binary_op() {
    let ir = gen_ok("fn add(a:i64,b:i64)->i64=a+b");
    assert!(ir.contains("add i64"));
}

#[test]
fn test_valid_nested_if() {
    let ir = gen_ok("fn test(a:i64)->i64{I a>0{I a>10{return 2} else {return 1}} else {return 0}}");
    assert!(ir.contains("br i1"));
}

#[test]
fn test_valid_loop() {
    let ir = gen_ok("fn test()->i64{x:=10;L _:x>0{x=x-1};x}");
    assert!(ir.contains("loop.start"));
}

#[test]
fn test_valid_multiple_functions() {
    let ir = gen_ok("fn a(x:i64)->i64=x+1 fn b(x:i64)->i64=a(x)+1");
...
fn test_valid_struct_definition() {
    let ir = gen_ok("struct Pair{first:i64,second:i64}");
    assert!(ir.contains("%Pair = type { i64, i64 }"));
}

#[test]
fn test_valid_enum_definition() {
    let ir = gen_ok("enum Color{Red,Green,Blue}");
    assert!(ir.contains("Color") || ir.contains("type {"));
}

#[test]
fn test_valid_self_recursion() {
    let ir = gen_ok("fn fib(n:i64)->i64=n<2?n:@(n-1)+@(n-2)");
    assert!(ir.contains("call i64 @fib"));
}

#[test]
fn test_valid_nested_struct() {
    let ir = gen_ok("struct Inner{val:i64} struct Outer{inner:Inner,extra:i64}");
...
fn test_error_in_else_branch() {
    let err = gen_err("fn test(x:i64)->i64{I x>0{return 1} else {return unknown_var}}");
    assert!(err.contains("unknown_var"), "Error: {}", err);
}

#[test]
fn test_error_in_then_branch() {
    let err = gen_err("fn test(x:i64)->i64{I x>0{return no_such_thing} else {return 0}}");
    assert!(err.contains("no_such_thing"), "Error: {}", err);
}

#[test]
fn test_error_nested_function_call() {
    let err = gen_err("fn inc(x:i64)->i64=x+1 fn main()->i64=inc(missing_var)");
...
fn test_error_multiple_undefined_stops_at_first() {
    ...
    let module = parse("fn test()->i64{return a+b}").unwrap();
    let mut gen = CodeGenerator::new("test");
    let result = gen.generate_module(&module);
...
fn test_function_with_no_params() {
    let ir = gen_ok("fn constant()->i64=42");
    assert!(ir.contains("define i64 @constant()"));
}

#[test]
fn test_function_with_many_params() {
    let ir = gen_ok("fn multi(a:i64,b:i64,c:i64,d:i64)->i64=a+b+c+d");
...
fn test_string_literal() {
    let ir = gen_ok("fn test()->str=\"hello\"");
    assert!(ir.contains("@test") || ir.contains("define"));
}

#[test]
fn test_array_literal_codegen() {
    let ir = gen_ok("fn test()->*i64=[1,2,3]");
    assert!(ir.contains("alloca"));
}

#[test]
fn test_comparison_operators() {
    let ir = gen_ok("fn test(a:i64,b:i64)->bool=a<b");
...
fn test_modulo_operator() {
    let ir = gen_ok("fn test(a:i64,b:i64)->i64=a%b");
...
#[test]
fn test_nested_loop() {
    let ir = gen_ok("fn test()->i64{x:= mut 0;L i:0..3{L j:0..3{x=x+1}};x}");
    assert!(ir.contains("br") || ir.contains("loop"));
}

#[test]
fn test_loop_with_break() {
    let ir = gen_ok("fn test()->i64{x:=0;L _:true{I x>10{B};x=x+1};x}");
    assert!(ir.contains("loop.end") || ir.contains("br"));
}

#[test]
fn test_loop_with_continue() {
    let ir = gen_ok("fn test()->i64{x:=0;L i:0..10{I i%2==0{C};x=x+1};x}");
    assert!(ir.contains("br") || ir.contains("loop"));
}
...
#[test]
fn test_match_basic() {
    let ir = gen_ok("fn test(x:i64)->i64{match x{0=>100,1=>200,_=>0}}");
    assert!(ir.contains("@test") || ir.contains("define"));
}
...
fn test_struct_construction() {
    let ir = gen_ok("struct Point{x:i64,y:i64} fn make()->Point{return Point{x:1,y:2}}");
    assert!(ir.contains("Point") || ir.contains("define"));
}

#[test]
fn test_struct_field_access() {
    let ir = gen_ok("struct Point{x:i64,y:i64} fn get_x(p:Point)->i64=p.x");
    assert!(ir.contains("getelementptr") || ir.contains("extractvalue") || ir.contains("GEP"));
}

#[test]
fn test_multi_field_struct() {
    let ir = gen_ok("struct Big{a:i64,b:i64,c:i64,d:i64,e:i64}");
...
fn test_expression_body_function() {
    let ir = gen_ok("fn double(x:i64)->i64=x*2");
    assert!(ir.contains("mul i64"));
}

#[test]
fn test_block_body_function() {
    let ir = gen_ok("fn double(x:i64)->i64{return x*2}");
    assert!(ir.contains("mul i64"));
}

#[test]
fn test_function_calling_function() {
    let ir = gen_ok("fn inc(x:i64)->i64=x+1 fn double_inc(x:i64)->i64=inc(inc(x))");
    assert!(ir.contains("call i64 @inc"));
}

#[test]
fn test_function_with_local_vars() {
    let ir = gen_ok("fn test(x:i64)->i64{a:=x+1;b:=a+1;return b}");
    assert!(ir.contains("@test") || ir.contains("define"));
}
...
#[test]
fn test_simple_enum() {
    let ir = gen_ok("enum Dir{North,South,East,West}");
...
fn test_enum_with_many_variants() {
    ...
    let ir = gen_ok("enum Status{A1,B1,C1,D1,E1,F1,H1,K1}");
...
fn test_error_message_contains_variable_name() {
    let err = gen_err("fn test()->i64{return missing_value}");
    assert!(
...
fn test_error_message_contains_function_name() {
    let err = gen_err("fn main()->i64=absent_func()");
    assert!(
...
fn test_error_message_has_field_info() {
    let err = gen_err("struct Point{x:i64,y:i64} fn test(p:Point)->i64=p.nonexistent");
    assert!(
...
#[test]
fn test_unary_minus() {
    let ir = gen_ok("fn neg(x:i64)->i64=0-x");
    assert!(ir.contains("sub i64"));
}

#[test]
fn test_logical_and() {
    let ir = gen_ok("fn test(a:bool,b:bool)->bool=a&&b");
    assert!(ir.contains("@test") || ir.contains("define"));
}
...
fn test_deeply_nested_expression() {
    let ir = gen_ok("fn test(x:i64)->i64=((((x+1)+2)+3)+4)");
    assert!(ir.contains("add i64"));
}

#[test]
fn test_mixed_operators() {
    let ir = gen_ok("fn test(a:i64,b:i64)->i64=a*b+a/b-a%b");
    assert!(ir.contains("mul i64"));
    assert!(ir.contains("sdiv") || ir.contains("div"));
}
...
fn test_chained_comparisons() {
    let ir = gen_ok("fn in_range(x:i64)->bool=x>0&&x<100");
...
fn test_multiple_structs() {
    let ir = gen_ok("struct A{x:i64} struct B{y:i64} struct C{z:i64}");
...
fn test_struct_and_function() {
    let ir = gen_ok("struct Point{x:i64,y:i64} fn origin()->Point{return Point{x:0,y:0}}");
...
fn test_enum_and_function() {
    let ir = gen_ok("enum MyBool{Yes,No} fn to_int(b:MyBool)->i64{match b{Yes=>1,No=>0}}");
...
fn test_mutable_variable() {
    let ir = gen_ok("fn test()->i64{x:= mut 0;x=42;return x}");
    assert!(ir.contains("store") || ir.contains("alloca"));
}

#[test]
fn test_compound_assignment() {
    let ir = gen_ok("fn test()->i64{x:= mut 10;x=x+5;return x}");
    assert!(ir.contains("add i64"));
}
...
fn test_error_in_let_value() {
    let err = gen_err("fn test()->i64{x:=missing_fn();return x}");
    assert!(err.contains("missing_fn"), "Error: {}", err);
}

#[test]
fn test_error_in_while_condition() {
    let err = gen_err("fn test()->i64{L _:unknown_pred(){B};return 0}");
    assert!(err.contains("unknown_pred"), "Error: {}", err);
}

#[test]
fn test_error_in_loop_body() {
    let err = gen_err("fn test()->i64{x:=0;L _:x<10{y:=missing();x=x+1};return x}");
...
fn test_trait_definition() {
    let ir = gen_ok("trait Printable{fn to_str(self)->str}");
...
#[test]
fn test_impl_block() {
    let ir = gen_ok("struct Point{x:i64,y:i64} impl Point{fn sum(self)->i64=self.x+self.y}");
...
fn test_pipe_operator() {
    let ir = gen_ok("fn double(x:i64)->i64=x*2 fn test(x:i64)->i64=x|>double");
    assert!(ir.contains("call i64 @double"));
}

#[test]
fn test_nested_call() {
    let ir = gen_ok("fn a(x:i64)->i64=x+1 fn b(x:i64)->i64=a(a(a(x)))");
...
fn test_codegen_error_undefined_var_format() {
    let module = parse("fn test()->i64{return xyz}").unwrap();
    let mut gen = CodeGenerator::new("test");
    let result = gen.generate_module(&module);
    assert!(result.is_err());
...
fn test_codegen_error_undefined_fn_format() {
    let module = parse("fn main()->i64=nope()").unwrap();
    let mut gen = CodeGenerator::new("test");
    let result = gen.generate_module(&module);
    assert!(result.is_err());
...
fn test_struct_with_bool_field() {
    let ir = gen_ok("struct Flag{active:bool,value:i64}");
    assert!(ir.contains("%Flag = type"));
}

#[test]
fn test_struct_with_pointer_field() {
    let ir = gen_ok("struct Node{value:i64,next:*i64}");
...
#[test]
fn test_for_range() {
    let ir = gen_ok("fn sum()->i64{s:=0;L i:0..10{s=s+i};s}");
    assert!(ir.contains("loop") || ir.contains("br"));
}

#[test]
fn test_for_range_variable_bounds() {
    let ir = gen_ok("fn sum(n:i64)->i64{s:=0;L i:0..n{s=s+i};s}");
    assert!(ir.contains("loop") || ir.contains("br"));
}
...
fn test_lambda_in_function() {
    ...
    let ir = gen_ok("fn test()->i64{f:=|x:i64|{x+1};f(41)}");
    assert!(ir.contains("@test") || ir.contains("define"));
}
...
fn test_match_enum_variants() {
    ...
    let ir = gen_ok("enum Dir{North,South} fn test(d:Dir)->i64{match d{North=>1,South=>2}}");
    assert!(ir.contains("@test") || ir.contains("define"));
}
...
fn test_early_return() {
    let ir = gen_ok("fn test(x:i64)->i64{I x<0{return 0};return x}");
    assert!(ir.contains("ret i64"));
}
