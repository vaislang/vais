...
pub fn declare_builtins<'ctx>(context: &'ctx Context, module: &Module<'ctx>) -> Result<(), String> {
    ...
    {
        let memcpy_fn = module.get_function("memcpy").ok_or("ICE: builtin memcpy must be declared before use")?;
        ...
        let result = builder
            .build_call(
                memcpy_fn,
                &[
                    func.get_nth_param(0).ok_or("ICE: builtin function missing parameter")?.into(),
                    func.get_nth_param(1).ok_or("ICE: builtin function missing parameter")?.into(),
                    func.get_nth_param(2).ok_or("ICE: builtin function missing parameter")?.into(),
                ],
                "result",
            )
        ...
        builder.build_return(Some(&result)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    {
        let fopen_fn = module.get_function("fopen").ok_or("ICE: builtin fopen must be declared before use")?;
    ...
    // get_stdin() -> FILE* (returns stdin stream)
    {
        ...
        let val = builder
            ...
            .map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        builder.build_return(Some(&val)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
...
fn define_simd_builtins<'ctx>(context: &'ctx Context, module: &Module<'ctx>) -> Result<(), String> {
    let builder = context.create_builder();
    let i8_ptr = context.i8_type().ptr_type(AddressSpace::default());
    let i32_type = context.i32_type();
    let i64_type = context.i64_type();
    ...
    // vec4i32(a, b, c, d) -> i8* (heap-allocated [i32 x 4])
    {
        ...
        for i in 0..4u32 {
            ...
            builder
                .build_store(gep, func.get_nth_param(i).ok_or("ICE: builtin function missing parameter")?.into_int_value())
                .map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&ptr)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // vec4f32(a, b, c, d) -> i8* (heap-allocated [float x 4])
    {
        ...
        for i in 0..4u32 {
            ...
            let param = func.get_nth_param(i).ok_or("ICE: builtin function missing parameter")?.into_float_value();
            let f32_val = builder.build_float_trunc(param, f32_type, "trunc").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            builder.build_store(gep, f32_val).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&ptr)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    for (name, op) in &[
        ...
        let a_ptr = builder
            .build_pointer_cast(
                func.get_nth_param(0).ok_or("ICE: builtin function missing parameter")?.into_pointer_value(),
        ...
        let b_ptr = builder
            .build_pointer_cast(
                func.get_nth_param(1).ok_or("ICE: builtin function missing parameter")?.into_pointer_value(),
                i32_type.ptr_type(AddressSpace::default()),
                "b",
            )
            .map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        ...
        for i in 0..4u32 {
            ...
            let a_gep = unsafe { builder.build_gep(i32_type, a_ptr, &[idx], "a_gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
            let b_gep = unsafe { builder.build_gep(i32_type, b_ptr, &[idx], "b_gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
            let a_val = builder
            ...
            let b_val = builder
                ...
                .into_int_value();
            let result = match *op {
                "add" => builder.build_int_add(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                "sub" => builder.build_int_sub(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                "mul" => builder.build_int_mul(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                _ => {
                    eprintln!(
                        "[ICE] unexpected SIMD operation in vec4i32: {} — using add as fallback",
                        op
                    );
                    builder.build_int_add(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                }
            };
            let o_gep = unsafe {
                builder
            ...
            builder.build_store(o_gep, result).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&out)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // simd_reduce_add_vec4i32: sum all 4 i32 elements -> i64
    {
        ...
        for i in 0..4u32 {
            ...
            let val = builder
                ...
                .map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                .into_int_value();
            let ext = builder.build_int_s_extend(val, i64_type, "ext").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            sum = builder.build_int_add(sum, ext, "sum").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&sum)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // simd_reduce_mul_vec4i32
    {
        ...
        for i in 0..4u32 {
            ...
            let val = builder
                ...
                .into_int_value();
            let ext = builder.build_int_s_extend(val, i64_type, "ext").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            prod = builder.build_int_mul(prod, ext, "prod").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&prod)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    for (name, op) in &[
        ...
        for i in 0..4u32 {
            let idx = i32_type.const_int(i as u64, false);
            let a_gep = unsafe { builder.build_gep(f32_type, a_ptr, &[idx], "a_gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
            let b_gep = unsafe { builder.build_gep(f32_type, b_ptr, &[idx], "b_gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
            let a_val = builder
            ...
            let b_val = builder
                ...
                .into_float_value();
            let result = match *op {
                "fadd" => builder.build_float_add(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                "fsub" => builder.build_float_sub(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                "fmul" => builder.build_float_mul(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                "fdiv" => builder.build_float_div(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?,
                _ => {
                    eprintln!(
                        "[ICE] unexpected SIMD operation in vec4f32: {} — using fadd as fallback",
                        op
                    );
                    builder.build_float_add(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                }
            };
            let o_gep = unsafe {
                builder
    ...
    // simd_reduce_add_vec4f32: sum all 4 f32 elements -> f64
    {
        ...
        for i in 0..4u32 {
            ...
            let val = builder
                ...
                .map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                .into_float_value();
            let ext = builder.build_float_ext(val, f64_type, "ext").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            sum = builder.build_float_add(sum, ext, "sum").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&sum)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // simd_reduce_mul_vec4f32
    {
        ...
        for i in 0..4u32 {
            ...
            let val = builder
                ...
                .into_float_value();
            let ext = builder.build_float_ext(val, f64_type, "ext").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            prod = builder.build_float_mul(prod, ext, "prod").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&prod)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // simd_dot_vec4f32(a, b) -> f64: sum of element-wise products
    {
        ...
        for i in 0..4u32 {
            ...
            let b_val = builder
                ...
                .map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                .into_float_value();
            let prod = builder.build_float_mul(a_val, b_val, "prod").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            let ext = builder.build_float_ext(prod, f64_type, "ext").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            sum = builder.build_float_add(sum, ext, "sum").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&sum)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // simd_get_vec4f32(ptr, idx) -> f64
    {
        ...
        let a_ptr = builder
            ...
            .map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        let idx = func.get_nth_param(1).ok_or("ICE: builtin function missing parameter")?.into_int_value();
        let gep = unsafe { builder.build_gep(f32_type, a_ptr, &[idx], "gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
        let val = builder
            .build_load(f32_type, gep, "val")
            .map_err(|e| format!("ICE: inkwell builtins: {e}"))?
            .into_float_value();
        let ext = builder.build_float_ext(val, f64_type, "ext").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        builder.build_return(Some(&ext)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
    ...
    // Vec2i64 binary ops
    {
        ...
        for i in 0..2u32 {
            let idx = i32_type.const_int(i as u64, false);
            let a_gep = unsafe { builder.build_gep(i64_type, a_ptr, &[idx], "a_gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
            let b_gep = unsafe { builder.build_gep(i64_type, b_ptr, &[idx], "b_gep").map_err(|e| format!("ICE: inkwell builtins: {e}"))? };
            let a_val = builder
            ...
            let b_val = builder
                ...
                .map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                .into_int_value();
            let result = builder.build_int_add(a_val, b_val, "r").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
            let o_gep = unsafe {
                builder
    ...
    // simd_reduce_add_vec2i64
    {
        ...
        for i in 0..2u32 {
            ...
            let val = builder
                ...
                .map_err(|e| format!("ICE: inkwell builtins: {e}"))?
                .into_int_value();
            sum = builder.build_int_add(sum, val, "sum").map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
        }
        builder.build_return(Some(&sum)).map_err(|e| format!("ICE: inkwell builtins: {e}"))?;
    }
