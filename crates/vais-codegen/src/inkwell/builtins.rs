...
fn define_simd_builtins<'ctx>(context: &'ctx Context, module: &Module<'ctx>) {
    ...
    {
        let fn_type = i8_ptr.fn_type(&[i32_type.into(), i32_type.into(), i32_type.into(), i32_type.into()], false);
        let func = module.add_function("vec4i32", fn_type, None);
        let entry = context.append_basic_block(func, "entry");
        builder.position_at_end(entry);
        // malloc(16) for 4 x i32
        let ptr = builder.build_call(malloc_fn, &[i64_type.const_int(16, false).into()], "ptr").unwrap()
            .try_as_basic_value().left().unwrap().into_pointer_value();
        let i32_ptr = builder.build_pointer_cast(ptr, i32_type.ptr_type(AddressSpace::default()), "i32ptr").unwrap();
        for i in 0..4u32 {
            let gep = unsafe { builder.build_gep(i32_type, i32_ptr, &[i32_type.const_int(i as u64, false)], "gep").unwrap() };
            builder.build_store(gep, func.get_nth_param(i).unwrap().into_int_value()).unwrap();
        }
    ...
    {
        let fn_type = i8_ptr.fn_type(&[f64_type.into(), f64_type.into(), f64_type.into(), f64_type.into()], false);
        let func = module.add_function("vec4f32", fn_type, None);
        let entry = context.append_basic_block(func, "entry");
        builder.position_at_end(entry);
        let ptr = builder.build_call(malloc_fn, &[i64_type.const_int(16, false).into()], "ptr").unwrap()
            .try_as_basic_value().left().unwrap().into_pointer_value();
        let f32_ptr = builder.build_pointer_cast(ptr, f32_type.ptr_type(AddressSpace::default()), "f32ptr").unwrap();
        for i in 0..4u32 {
            let gep = unsafe { builder.build_gep(f32_type, f32_ptr, &[i32_type.const_int(i as u64, false)], "gep").unwrap() };
    ...
    // vec2i64(a, b) -> i8*
    {
        ...
        let ptr = builder.build_call(malloc_fn, &[i64_type.const_int(16, false).into()], "ptr").unwrap()
            .try_as_basic_value().left().unwrap().into_pointer_value();
        let i64_ptr = builder.build_pointer_cast(ptr, i64_type.ptr_type(AddressSpace::default()), "i64ptr").unwrap();
        for i in 0..2u32 {
            let gep = unsafe { builder.build_gep(i64_type, i64_ptr, &[i32_type.const_int(i as u64, false)], "gep").unwrap() };
            builder.build_store(gep, func.get_nth_param(i).unwrap().into_int_value()).unwrap();
        }
        builder.build_return(Some(&ptr)).unwrap();
    }
    ...
    for (name, op) in &[("simd_add_vec4i32", "add"), ("simd_sub_vec4i32", "sub"), ("simd_mul_vec4i32", "mul")] {
        let fn_type = i8_ptr.fn_type(&[i8_ptr.into(), i8_ptr.into()], false);
        let func = module.add_function(name, fn_type, None);
        let entry = context.append_basic_block(func, "entry");
        builder.position_at_end(entry);

        let a_ptr = builder.build_pointer_cast(func.get_nth_param(0).unwrap().into_pointer_value(), i32_type.ptr_type(AddressSpace::default()), "a").unwrap();
        let b_ptr = builder.build_pointer_cast(func.get_nth_param(1).unwrap().into_pointer_value(), i32_type.ptr_type(AddressSpace::default()), "b").unwrap();
        let out = builder.build_call(malloc_fn, &[i64_type.const_int(16, false).into()], "out").unwrap()
            .try_as_basic_value().left().unwrap().into_pointer_value();
        let out_i32 = builder.build_pointer_cast(out, i32_type.ptr_type(AddressSpace::default()), "out_i32").unwrap();
        ...
        for i in 0..4u32 {
            ...
            let a_val = builder.build_load(i32_type, a_gep, "a_val").unwrap().into_int_value();
            let b_val = builder.build_load(i32_type, b_gep, "b_val").unwrap().into_int_value();
            let result = match *op {
            ...
            let o_gep = unsafe { builder.build_gep(i32_type, out_i32, &[idx], "o_gep").unwrap() };
    ...
    // simd_reduce_add_vec4i32: sum all 4 i32 elements -> i64
    {
        ...
        for i in 0..4u32 {
            let gep = unsafe { builder.build_gep(i32_type, a_ptr, &[i32_type.const_int(i as u64, false)], "gep").unwrap() };
            let val = builder.build_load(i32_type, gep, "val").unwrap().into_int_value();
    ...
    for (name, op) in &[("simd_add_vec4f32", "fadd"), ("simd_sub_vec4f32", "fsub"), ("simd_mul_vec4f32", "fmul"), ("simd_div_vec4f32", "fdiv")] {
        let fn_type = i8_ptr.fn_type(&[i8_ptr.into(), i8_ptr.into()], false);
        let func = module.add_function(name, fn_type, None);
        let entry = context.append_basic_block(func, "entry");
        builder.position_at_end(entry);

        let a_ptr = builder.build_pointer_cast(func.get_nth_param(0).unwrap().into_pointer_value(), f32_type.ptr_type(AddressSpace::default()), "a").unwrap();
        let b_ptr = builder.build_pointer_cast(func.get_nth_param(1).unwrap().into_pointer_value(), f32_type.ptr_type(AddressSpace::default()), "b").unwrap();
        let out = builder.build_call(malloc_fn, &[i64_type.const_int(16, false).into()], "out").unwrap()
            .try_as_basic_value().left().unwrap().into_pointer_value();
        let out_f32 = builder.build_pointer_cast(out, f32_type.ptr_type(AddressSpace::default()), "out_f32").unwrap();
        ...
        for i in 0..4u32 {
            ...
            let a_val = builder.build_load(f32_type, a_gep, "a_val").unwrap().into_float_value();
            let b_val = builder.build_load(f32_type, b_gep, "b_val").unwrap().into_float_value();
            let result = match *op {
            ...
            let o_gep = unsafe { builder.build_gep(f32_type, out_f32, &[idx], "o_gep").unwrap() };
            builder.build_store(o_gep, result).unwrap();
        }
        builder.build_return(Some(&out)).unwrap();
    }
    ...
    // simd_reduce_add_vec4f32: sum all 4 f32 elements -> f64
    {
        ...
        for i in 0..4u32 {
            let gep = unsafe { builder.build_gep(f32_type, a_ptr, &[i32_type.const_int(i as u64, false)], "gep").unwrap() };
            let val = builder.build_load(f32_type, gep, "val").unwrap().into_float_value();
    ...
    // Vec2i64 binary ops
    {
        ...
        let entry = context.append_basic_block(func, "entry");
        builder.position_at_end(entry);
        let a_ptr = builder.build_pointer_cast(func.get_nth_param(0).unwrap().into_pointer_value(), i64_type.ptr_type(AddressSpace::default()), "a").unwrap();
        let b_ptr = builder.build_pointer_cast(func.get_nth_param(1).unwrap().into_pointer_value(), i64_type.ptr_type(AddressSpace::default()), "b").unwrap();
        let out = builder.build_call(malloc_fn, &[i64_type.const_int(16, false).into()], "out").unwrap()
            .try_as_basic_value().left().unwrap().into_pointer_value();
        let out_i64 = builder.build_pointer_cast(out, i64_type.ptr_type(AddressSpace::default()), "out_i64").unwrap();
        for i in 0..2u32 {
            let idx = i32_type.const_int(i as u64, false);
            ...
            let a_val = builder.build_load(i64_type, a_gep, "a_val").unwrap().into_int_value();
            let b_val = builder.build_load(i64_type, b_gep, "b_val").unwrap().into_int_value();
            let result = builder.build_int_add(a_val, b_val, "r").unwrap();
            let o_gep = unsafe { builder.build_gep(i64_type, out_i64, &[idx], "o_gep").unwrap() };
            builder.build_store(o_gep, result).unwrap();
        }
        builder.build_return(Some(&out)).unwrap();
    }
    ...
    // simd_reduce_add_vec2i64
    {
        ...
        let mut sum = i64_type.const_int(0, false);
        for i in 0..2u32 {
            let gep = unsafe { builder.build_gep(i64_type, a_ptr, &[i32_type.const_int(i as u64, false)], "gep").unwrap() };
            let val = builder.build_load(i64_type, gep, "val").unwrap().into_int_value();
