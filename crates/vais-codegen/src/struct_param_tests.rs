...
mod tests {
    ...
    fn test_struct_argument_passing_generates_load() {
        ...
        let call_expr = Expr::Call {
            func: Box::new(Spanned::new(
                Expr::Ident("consume_point".to_string()),
                Span::default(),
            )),
        ...
        let (_, call_ir) = codegen
            .generate_expr(&Spanned::new(call_expr, Span::default()), &mut counter)
            .unwrap();
        ...
        assert!(
            call_ir.contains("call i64 @consume_point(%Point"),
    ...
    fn test_multiple_struct_arguments() {
        ...
        assert!(
            fn_ir.contains("%Point %p1"),
            "First parameter should be value type, got:\n{}",
            fn_ir
        );
        assert!(
            fn_ir.contains("%Point %p2"),
        ...
        let call_expr = Expr::Call {
            func: Box::new(Spanned::new(
                Expr::Ident("add_points".to_string()),
                Span::default(),
            )),
            args: vec![
                Spanned::new(
                ...
                Spanned::new(
                    Expr::StructLit {
                        ...
                        fields: vec![
                            (
                                Spanned::new("x".to_string(), Span::default()),
                                Spanned::new(Expr::Int(3), Span::default()),
                            ),
                            (
                                Spanned::new("y".to_string(), Span::default()),
