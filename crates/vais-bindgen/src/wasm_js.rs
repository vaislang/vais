...
impl WasmJsBindgen {
    ...
    pub fn generate_js(&self) -> String {
        ...
        js.push_str(&format!("// Auto-generated WASM bindings for {}\n", self.module_name));
        js.push_str("// Generated by vais-bindgen\n\n");

        // Generate import object builder
        js.push_str("/**\n * Create the import object for WASM instantiation.\n");
        js.push_str(" * Override individual functions to provide custom JS implementations.\n */\n");
        ...
        for exp in &self.exports {
            if exp.params.is_empty() {
                ...
                let params = exp.params.iter().map(|(n, _)| n.clone()).collect::<Vec<_>>().join(", ");
                js.push_str(&format!(
                    "    {}: ({}) => instance.exports.{}({}),\n",
...
mod tests {
    ...
    fn test_multiple_import_modules() {
        ...
        gen.add_import(WasmImportInfo {
            ...
            params: vec!["i32".to_string(), "i32".to_string(), "i32".to_string(), "i32".to_string()],
            return_type: Some("i32".to_string()),
        });
        let js = gen.generate_js();
        assert!(js.contains("\"env\""));
