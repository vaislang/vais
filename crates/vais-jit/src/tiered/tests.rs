...
fn test_interpreter_function_call() {
    let source = "fn add(a:i64,b:i64)->i64{a+b} fn main()->i64{add(3,4)}";
    let ast = parse(source).unwrap();
...
fn test_profiling_execution_count() {
    let source = "fn foo()->i64{1} fn main()->i64{foo();foo();foo();0}";
    let ast = parse(source).unwrap();
...
fn test_tier_promotion_detection() {
    ...
    let source = "fn hot()->i64{1} fn main()->i64{hot();hot();hot();0}";
    let ast = parse(source).unwrap();
...
fn test_function_stats() {
    let source = "fn foo()->i64{1} fn main()->i64{foo();0}";
    let ast = parse(source).unwrap();
...
fn test_tier_promotion_baseline_to_optimizing() {
    ...
    let source = "fn hot()->i64{42} fn main()->i64{hot()}";
    let ast = parse(source).unwrap();
...
fn test_branch_profiling() {
    let source = "fn branch_test(x:i64)->i64{I x>5{1} else {0}} fn main()->i64{branch_test(10)}";
    let ast = parse(source).unwrap();
...
fn test_tiered_jit_with_custom_thresholds() {
    ...
    let source = "fn compute()->i64{10+20+30} fn main()->i64{compute()}";
    let ast = parse(source).unwrap();
...
fn test_deopt_blacklist() {
    let source = "fn unstable()->i64{1} fn main()->i64{unstable()}";
    let ast = parse(source).unwrap();
...
fn test_dynamic_tier_promotion_with_score() {
    ...
    let source = "fn work()->i64{x:=0;L{I x>=20{return x} x:=x+1}0} fn main()->i64{work()}";
    let ast = parse(source).unwrap();
...
fn test_total_loop_iterations() {
    let source = "fn looper()->i64{x:=0;L{I x>=10{return x} x:=x+1}0} fn main()->i64{looper()}";
    let ast = parse(source).unwrap();
...
fn test_get_all_stats() {
    let source = "fn foo()->i64{1} fn bar()->i64{2} fn main()->i64{foo()+bar()}";
    let ast = parse(source).unwrap();
...
fn test_branch_bias_score() {
    let source = "fn biased(x:i64)->i64{I x>0{1} else {0}} fn main()->i64{biased(10)}";
    let ast = parse(source).unwrap();
...
fn test_last_promoted_at() {
    let source = "fn func()->i64{1} fn main()->i64{func()}";
    let ast = parse(source).unwrap();
