...
impl OpenCLGenerator {
    ...
    fn generate_helper_function(&mut self, func: &Function) -> GpuResult<()> {
        ...
        for (i, param) in func.params.iter().enumerate() {
            ...
            self.emit(&format!("{} {}", self.type_to_opencl(&param.ty.node), param.name.node));
        }
    ...
    fn generate_struct(&mut self, name: &str, fields: &[vais_ast::Field]) -> GpuResult<()> {
        ...
        for field in fields {
            self.emit_indent();
            self.emit_line(&format!("{} {};", self.type_to_opencl(&field.ty.node), field.name.node));
        }
    ...
    fn generate_stmt(&mut self, stmt: &Stmt) -> GpuResult<()> {
        match stmt {
            Stmt::Let { name, ty, value, .. } => {
                self.emit_indent();
                if let Some(t) = ty {
                    self.emit(&format!("{} {} = ", self.type_to_opencl(&t.node), name.node));
                } else {
...
mod tests {
    ...
    fn test_opencl_generator_basic() {
        ...
        assert_eq!(gen.type_to_opencl(&Type::Named { name: "i64".to_string(), generics: vec![] }), "long");
        assert_eq!(gen.type_to_opencl(&Type::Named { name: "f64".to_string(), generics: vec![] }), "double");
    }
}
