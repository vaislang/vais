    ...
    fn generate_compute_shader(
        ...
        for param in &func.params {
            ...
            param_types.push((
                ...
                GpuType::Ptr(Box::new(GpuType::F64)),
            ));
        }
    ...
    fn generate_helper_function(&mut self, func: &Function) -> GpuResult<()> {
        ...
        for (i, param) in func.params.iter().enumerate() {
            ...
            self.emit(&format!(
                "{}: {}",
                param.name.node,
                self.type_to_wgsl(&param.ty.node)
            ));
        }
    ...
    fn generate_struct(&mut self, name: &str, fields: &[vais_ast::Field]) -> GpuResult<()> {
        ...
        for field in fields {
            ...
            self.emit_line(&format!(
                "{}: {},",
                field.name.node,
                self.type_to_wgsl(&field.ty.node)
            ));
        }
...
pub fn generate_host_code(kernels: &[GpuKernel], wgsl_code: &str) -> String {
    ...
    for kernel in kernels {
        code.push_str(&format!(
            "async function run_{}(device: GPUDevice",
            kernel.name
        ));
