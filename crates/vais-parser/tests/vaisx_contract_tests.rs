...
fn test_vx_state_string_literal() {
    let source = r#"fn init() { name := __vx_state("world") }"#;
...
fn test_vx_derived_string_concat() {
    let source = r#"fn init() { greeting := __vx_derived(|| { "Hello, " + name + "!" }) }"#;
...
fn test_vx_effect_simple() {
    let source = r#"fn init() { __vx_effect(|| { console_log("changed") }) }"#;
...
fn test_vx_emit_simple() {
    let source = r#"fn handleClick() { __vx_emit("select") }"#;
...
fn test_vx_emit_with_args() {
    let source = r#"fn handleClick() { __vx_emit("select", user) }"#;
...
fn test_wasm_attribute_function() {
    let source = r#"
        #[wasm]
        fn processData(raw: Vec<f64>) -> Vec<f64> {
            raw
        }
    "#;
...
fn test_server_attribute_with_args() {
    let source = r#"
        ...
        A fn sensitiveAction(data: str) -> str {
            data
        }
    "#;
...
fn test_full_component_desugar() {
    let source = r#"
        ...
        #[server]
        A fn loadData() -> Vec<i64> {
            data
        }

        #[wasm]
        fn heavyCompute(input: Vec<f64>) -> f64 {
            0.0
        }
    "#;
