...
impl LlvmEmitter {
    ...
    fn emit_module(&mut self, module: &MirModule) -> String {
        // Module preamble
        self.output.push_str(&format!("; ModuleID = '{}'\n", module.name));
    ...
    fn emit_body(&mut self, body: &Body) {
        ...
        self.output.push_str(&format!("define {} @{}(", ret_type, body.name));
        ...
        for (i, param_ty) in body.params.iter().enumerate() {
            ...
            let name = self.local_names.get(&param_local).cloned().unwrap_or_else(|| format!("%_arg{}", i));
        ...
        for (i, local_decl) in body.locals.iter().enumerate() {
            ...
            let name = self.local_names.get(&Local(i as u32)).cloned().unwrap_or_else(|| format!("%_{}", i));
            self.output.push_str(&format!("  {}.addr = alloca {}\n", name, llvm_ty));
        }

        // Return place alloca
        if body.return_type != MirType::Unit {
            let ret_llvm = self.mir_type_to_llvm(&body.return_type);
            self.output.push_str(&format!("  %_0.addr = alloca {}\n", ret_llvm));
        }
    ...
    fn emit_assign(&mut self, place: &Place, rvalue: &Rvalue, body: &Body) {
        ...
        match rvalue {
            Rvalue::Use(operand) => {
                ...
                self.output.push_str(&format!("  {} = add {} 0, {}\n", dest_name, llvm_ty, val));
            }
            ...
            Rvalue::UnaryOp(op, operand) => {
                ...
                match op {
                    ...
                    UnOp::Not => {
                        self.output.push_str(&format!("  {} = xor {} {}, -1\n",
                            dest_name, llvm_ty, val));
                    }
                }
            }
        ...
        match operand {
            ...
            Operand::Constant(c) => {
                match c {
                    ...
                    Constant::Bool(v) => if *v { "1".to_string() } else { "0".to_string() },
    ...
    fn emit_terminator(&mut self, term: &Terminator, body: &Body) {
        match term {
            Terminator::Goto(bb) => {
                let label = if bb.0 == 0 { "entry".to_string() } else { format!("bb{}", bb.0) };
            ...
            Terminator::SwitchInt { discriminant, targets, otherwise } => {
                ...
                if targets.len() == 1 {
                    ...
                    let t_label = if target.0 == 0 { "entry".to_string() } else { format!("bb{}", target.0) };
                    let f_label = if otherwise.0 == 0 { "entry".to_string() } else { format!("bb{}", otherwise.0) };
                    self.output.push_str(&format!("  br i1 {}, label %{}, label %{}\n",
                        cmp_name, t_label, f_label));
                } else {
                    // Multi-target = switch
                    let o_label = if otherwise.0 == 0 { "entry".to_string() } else { format!("bb{}", otherwise.0) };
                    self.output.push_str(&format!("  switch i64 {}, label %{} [\n",
                        disc_val, o_label));
                    for (val, target) in targets {
                        let t_label = if target.0 == 0 { "entry".to_string() } else { format!("bb{}", target.0) };
                        self.output.push_str(&format!("    i64 {}, label %{}\n", val, t_label));
                    }
                    self.output.push_str("  ]\n");
                }
            }
            ...
            Terminator::TailCall { func, args } => {
                ...
                self.output.push_str(&format!("  {} = musttail call {} @{}(",
            ...
            Terminator::Assert { cond, expected, msg, target } => {
                ...
                self.output.push_str(&format!("  {} = icmp eq i1 {}, {}\n",
                    cmp_name, cond_val, expected_val));
                self.output.push_str(&format!("  ; assert: {}\n", msg));
                let t_label = if target.0 == 0 { "entry".to_string() } else { format!("bb{}", target.0) };
                self.output.push_str(&format!("  br i1 {}, label %{}, label %assert_fail\n",
                    cmp_name, t_label));
            }
        }
    }
    ...
    fn local_name(&self, local: Local) -> String {
        self.local_names.get(&local).cloned().unwrap_or_else(|| format!("%_{}", local.0))
    }
    ...
    fn operand_type(&self, operand: &Operand, body: &Body) -> MirType {
        match operand {
            Operand::Copy(place) | Operand::Move(place) => {
                self.local_type(place.local, body)
            }
    ...
    fn mir_type_to_llvm(&self, ty: &MirType) -> String {
        match ty {
            ...
            MirType::Function { params, ret } => {
                ...
                let param_tys: Vec<String> = params.iter().map(|t| self.mir_type_to_llvm(t)).collect();
    ...
    fn binop_to_llvm(&self, op: &BinOp, ty: &MirType) -> &'static str {
        ...
        match op {
            ...
            BinOp::Rem => if is_float { "frem" } else { "srem" },
            BinOp::BitAnd => "and",
            BinOp::BitOr => "or",
            BinOp::BitXor => "xor",
            BinOp::Shl => "shl",
            ...
            BinOp::Eq => "icmp eq",
            BinOp::Ne => "icmp ne",
            BinOp::Lt => if is_float { "fcmp olt" } else { "icmp slt" },
            BinOp::Le => if is_float { "fcmp ole" } else { "icmp sle" },
            BinOp::Gt => if is_float { "fcmp ogt" } else { "icmp sgt" },
            BinOp::Ge => if is_float { "fcmp oge" } else { "icmp sge" },
        }
    }
}
...
mod tests {
    ...
    fn test_emit_with_branch() {
        ...
        builder.assign(
            Place::local(cond),
            Rvalue::BinaryOp(BinOp::Lt, param.clone(), Operand::Constant(Constant::Int(0))),
        );
        builder.switch_int(
