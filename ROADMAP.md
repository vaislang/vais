...
> **상태**: 🔄 진행 중 (Stage 0~1 완료 — IR-level .o 캐싱 + Per-module codegen)
...
### Stage 2: Per-module .o 캐싱 + incremental linking (미구현)

**목표**: 변경되지 않은 모듈의 .o 재사용 + 변경 모듈만 재컴파일

- [ ] 모듈별 .ll → `clang -c` → .o (콘텐츠 해시 기반 캐시)
- [ ] 미변경 모듈: 캐시된 .o 재사용
- [ ] 최종: 모든 .o → `clang` (링크만) → 바이너리
- [ ] 병렬 컴파일: 모듈별 .o 생성을 rayon으로 병렬화
- **난이도**: 상 | **모델**: Opus 직접
...
| Stage 2 | 1 파일 변경 → 1개 .o만 재생성 + 나머지 캐시 재사용 | 미구현 |
| Stage 3 | 30K lines, 1 파일 변경: 571ms → 100ms 이하 | 미구현 |

### 우선순위

```
Stage 0 ✅ (.o 캐싱) ──→ Stage 1 ✅ (per-module codegen) ──→ Stage 2 (per-module caching) ──→ Stage 3 (검증)
```
