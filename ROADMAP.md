...
- **fixpoint_full 잔존 codegen 능력 갭**(부트스트랩 향 확장 후보, 2026-06-07 경계매핑서 식별):
  ① `(...)` 그룹화 미지원 — gen_factor가 `(`를 call로만 파싱, `(a<b)+(b<c)` 같은 그룹 산술 실패.
  ② `for` 루프 codegen 미구현(while만 — 실제 소스는 while 우회 가능). ③비교-as-value는 **해결**(FP12g,
  `return a==b`→icmp+zext). 부트스트랩 전 이 둘(① 그룹화, ② for) 추가하면 소스 커버리지 향상.
