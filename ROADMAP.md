...
| **Phase 49** | VaisDB 대응: 크로스 플랫폼 & SIMD | 📋 예정 — 플랫폼 조건부 상수, SIMD 벡터 연산 벤치마크 | - |
...
### Stage 0: 컴파일 타임 플랫폼 감지

**목표**: `#[cfg(os = "linux")]` / `#[cfg(os = "macos")]` 패턴 지원

- [ ] 빌드 시 `target_os` 감지 (컴파일러에 `--target` 정보 전달)
- [ ] 조건부 상수 정의 매커니즘 (cfg attribute 또는 comptime if)
- [ ] std/net.vais의 `AF_INET6`, `SO_REUSEPORT` 등 플랫폼별 분기
- [ ] std/signal.vais의 시그널 번호 플랫폼별 정의
- [ ] std/file.vais의 mmap 플래그 (`MAP_ANONYMOUS` vs `MAP_ANON`)
- **난이도**: 중 | **모델**: Opus 직접
