...
- [x] **error_suggestion_tests 수정** - 6개 실패 테스트 복구 (완료일: 2026-01-29)
  ...
  - 배열 인덱싱 타입 체크 추가
- [x] **통합 테스트 확장 Part 1** - E2E 테스트 프레임워크 구축 (완료일: 2026-01-29)
  - 컴파일 → clang → 실행 → 종료 코드/stdout 검증 자동화 파이프라인
  - 47개 E2E 테스트: 기본 프로그램, 산술, 함수, 재귀(@), if/else, match, 변수, 비교, 구조체, 열거형, 배열, 복합 프로그램
  - 컴파일 에러 검증, 엣지 케이스 (exit code 모듈로, 음수 반환) 포함
  - 발견된 코드겐 제한사항 문서화: 구조체/열거형 함수 인자 전달, 중첩 필드 접근, 열거형 variant 매칭
- [x] **통합 테스트 확장 Part 2** - 회귀 테스트 및 기능별 E2E 확장 (완료일: 2026-01-29)
  - 42개 신규 E2E 테스트 추가 (총 89개): 비트연산, 논리연산, 재귀 반복, 문자열 출력, 고급 재귀, 복합 제어흐름, 변수 스코핑, 배열 연산, 엣지케이스/회귀, 다중 구조체
  - 컴파일러 제한사항 발견 및 문서화: 루프 변수 바인딩 codegen 버그, 논리 NOT codegen 버그
