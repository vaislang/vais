...
- [ ] 4. **Group-I: Generic/Vec 회귀 + 잔여 (4건)** (Opus direct) [blockedBy: 1, 3]
  - **C1 (R-1) 2026-04-17 부분 진행**:
    ...
    - **R-1b 잔여**: stdlib `std/vec.vais`의 `vec_new() -> Vec<i64>` 내부 `Vec.with_capacity(8)` 호출은 peer(push)가 같은 파일에 없어서 여전히 link error. 근본 해결: TC `checker_expr/calls.rs:749-762` Vec/HashMap built-in static path에서 **return type unification 후 retroactive add_instantiation** hook 필요. examples/simple_vec_test.vais, examples/generic_vec_test.vais가 영향 받음
  ...
  - 완료 기준: R-1b 포함 4건 모두 runtime 통과 또는 assert_exit_code 전환. Recon-C S2/S2b/S5 runtime PASS. E2E 2596+ 유지, clippy 0/0
