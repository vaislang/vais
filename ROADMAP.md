...
| **63** | **실행 검증 강화** | 📋 예정 | IR 검증 → 바이너리 컴파일+실행+출력 E2E, std 라이브러리 실행 검증, 에러 메시지 UX |
...
- [ ] 1. E2E 테스트 헬퍼 — compile→execute→assert_stdout 파이프라인 구축 (Opus 직접)
- [ ] 2. 기존 IR-only 테스트 중 핵심 20개를 실행 기반으로 전환 (Sonnet 위임)
- [ ] 3. examples/ 디렉토리 전체 컴파일+실행 스모크 테스트 (Sonnet 위임)

### Stage 1: std 라이브러리 실행 검증

- [ ] 4. std/ 핵심 모듈 실행 테스트 — Vec, HashMap, String, Option, Result (Opus 직접)
- [ ] 5. std/ I/O 모듈 실행 테스트 — File, IO, Path (Sonnet 위임)
- [ ] 6. std/ 네트워크 모듈 스모크 테스트 — Net, HTTP (Sonnet 위임)

### Stage 2: 에러 메시지 UX 검증

- [ ] 7. 주요 에러 시나리오 20개 — 에러 메시지 출력 캡처 + 가독성 검증 (Sonnet 위임)
- [ ] 8. 에러 메시지 스냅샷 테스트 프레임워크 (Opus 직접)
