...
| **Phase 48** | VaisDB 대응: 타입 안전성 | 📋 예정 — Result<T,E> 제네릭화, 컨테이너 메모리 레이아웃 개선 | - |
...
> **상태**: 📋 예정
> **목표**: Result<T,E> 제네릭화 + 제네릭 컨테이너 메모리 레이아웃 개선
...
- [ ] enum variant에 제네릭 타입 파라미터 payload 지원 (codegen)
- [ ] Result<T, E>의 Ok(T), Err(E)에서 T, E가 임의 타입 (struct, enum, 기본형)
- [ ] `?` 연산자가 제네릭 Result<T, E>와 동작
- [ ] `unwrap()`, `map()`, `and_then()` 등 기존 메서드 제네릭 타입 대응
- [ ] 기존 i64 기반 Result 코드 호환성 (마이그레이션 가이드)
- **난이도**: 상 | **모델**: Opus 직접

### Stage 1: 제네릭 컨테이너 타입 안전 메모리 레이아웃

**목표**: Vec<T>, HashMap<K,V> 등이 내부적으로 실제 타입 크기로 저장

- [ ] Vec<T>의 내부 메모리가 `sizeof(T)` 기반 레이아웃 사용 (i64 고정 아님)
- [ ] HashMap<K,V> 체인 노드가 실제 K, V 타입 크기 사용
- [ ] BTreeMap<K,V> 노드의 타입 안전 레이아웃
- [ ] 제네릭 컨테이너 간 타입 전파 (예: `Vec<Result<Page, Error>>`)
- **난이도**: 상 | **모델**: Opus 직접

### Stage 2: 타입 안전성 E2E 검증

**목표**: 데이터베이스 시나리오 기반 타입 혼동 방지 검증

- [ ] `Vec<u32>` vs `Vec<u64>` 에 잘못된 타입 삽입 시 컴파일 에러
- [ ] `Result<Page, StorageError>` → `Result<Row, SqlError>` 변환 시 `map_err` 필수
- [ ] 중첩 제네릭: `HashMap<String, Vec<Result<T, E>>>` 동작 확인
- [ ] E2E 테스트 20개+ 추가
- **난이도**: 중 | **모델**: Opus 직접
