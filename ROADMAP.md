...
> **상태**: ✅ 완료
> **목표**: 컴파일러 에러 메시지 품질 향상 — help() 100% 커버리지, 다중 에러 수집, 다중 스팬 표시
> **영향도**: Medium — 사용자 경험(DX) 핵심

- [x] 1. help() 메시지 100% 커버리지 & 타입 변환 제안 활성화 (Sonnet 위임) ✅ 2026-02-14
  변경: types.rs (+28줄, 8개 누락 variant help 추가), diagnostics.rs (suggest_type_conversion 활성화)
- [x] 2. 미사용 변수/import 경고 강화 & 다중 에러 수집 (Opus 직접) ✅ 2026-02-14
  변경: lib.rs (+27줄, multi_error_mode/try_or_collect), checker_module.rs (2nd pass 다중 에러 수집), build.rs (+34줄, multi-error 출력)
- [x] 3. 에러 출력 포맷 개선 — 다중 스팬 & 노트 표시 (Sonnet 위임) ✅ 2026-02-14
  변경: types.rs (+62줄, secondary_spans() 7개 variant), error_report.rs (+33줄, format_error secondary_spans), error_formatter.rs (+4줄)
- [x] 4. E2E 에러 메시지 품질 테스트 추가 (Opus 직접) ✅ 2026-02-14
  변경: error_message_tests.rs (14→25개 테스트, help/secondary_spans/multi-error/error_code 검증)
진행률: 4/4 (100%)
...
> **상태**: ✅ 완료 (2026-02-14)
> **목표**: 최소 테스트만 보유한 12개 crate에 통합 테스트 추가, 전체 안정성 강화
> **영향도**: Medium — 회귀 방지, 리팩토링 안전망 확보

- [x] 1. 핵심 인프라 crate 테스트 (Sonnet) ✅ 2026-02-14
  변경: vais-ast +13, vais-bindgen +10, vais-dynload +10, vais-hotreload +10 통합 테스트
- [x] 2. 개발자 도구 crate 테스트 (Sonnet) ✅ 2026-02-14
  변경: vais-dap +10, vais-i18n +10, vais-plugin +10, vais-security +10 통합 테스트
- [x] 3. 에코시스템 crate 테스트 (Sonnet) ✅ 2026-02-14
  변경: vais-supply-chain +10, vais-testgen +10, vais-gc +10, vais-gpu +10 통합 테스트
- [x] 4. 테스트 커버리지 리포트 갱신 & CI 커버리지 임계값 설정 (Opus) ✅ 2026-02-14
  변경: codecov.yml 6개 컴포넌트로 재구성, ci.yml 커버리지 threshold 체크 추가
