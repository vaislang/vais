...
### P0 - 긴급: 기존 버그 수정 및 빌드 안정화

> 현재 실패하는 테스트와 빌드 에러를 모두 해결

#### stdlib 타입 자동 등록 수정
- [ ] **Box\<T\> 구조체 자동 등록** - 통합 테스트에서 Box\<T\> 타입이 자동 로드되지 않는 문제 수정
- [ ] **Rc\<T\> 구조체 자동 등록** - 통합 테스트에서 Rc\<T\> 타입이 자동 로드되지 않는 문제 수정
- [ ] **Future\<T\> 제네릭 구조체 등록** - Future\<T\> 타입이 타입 체커에 올바르게 등록되지 않는 문제 수정
- [ ] **통합 테스트 128/128 통과** - 현재 125/128 (3개 실패) → 전수 통과 달성

#### 빌드 안정화
- [ ] **Python 바인딩 release 빌드 수정** - PyO3 링킹 에러 해결, `cargo build --release` 전체 성공
