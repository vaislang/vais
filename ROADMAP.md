...
- [ ] **렉서: 보간 문자열 토큰 추가** - `"hello {name}"` 형태의 StringInterpolation 토큰
- [ ] **파서: 보간 표현식 파싱** - `{expr}` 내부를 표현식으로 파싱, 중첩 지원
- [ ] **코드젠: 보간 → printf/snprintf 변환** - LLVM IR에서 포맷 스트링 생성
- [ ] **println 빌트인 추가** - `println("x={x}, y={y}")` → puts + printf 조합
- [ ] **테스트** - 보간 문자열 E2E 테스트 (정수, 실수, 문자열, 표현식)
