...
> **상태**: 🔄 진행 중 (Stage 0 완료 — IR-level .o 캐싱 구현)
...
### Stage 1: Per-module codegen 분리 (미구현)

**목표**: merged AST → 모듈별 독립 .ll 파일 생성

- [ ] `generate_module()` → `generate_single_module(module_name, items)` 추가
- [ ] 모듈 간 extern 선언 자동 생성 (다른 모듈 함수/타입 참조)
- [ ] main.rs: 모듈별 AST 유지 (merge 대신 `Vec<(ModuleName, Module)>`)
- [ ] 각 모듈 → 독립 .ll 파일 출력
- [ ] 기존 single-file 모드 호환 유지
- **난이도**: 상 | **모델**: Opus 직접
...
| Stage 1 | 10-file 프로젝트 → 10개 .ll 파일 생성, 각각 컴파일 가능 | 미구현 |
