...
### Stage 1: 셀프호스트 에러 복구 파서

**목표**: 여러 에러를 한번에 보고하는 에러 복구 모드

- [ ] 파서 에러 복구 전략 구현 (synchronization tokens 기반)
- [ ] 첫 에러에서 중단하지 않고 최대 20개 에러 수집
- [ ] 에러 위치 정보(줄:열) 정확도 개선
- [ ] 에러 복구 E2E 테스트 10개
- **난이도**: 중 | **모델**: Opus 직접

### Stage 2: 클로저 캡처 개선 & 고차 함수 표준 패턴

**목표**: 클로저/람다의 실전 활용도 향상

- [ ] selfhost parser에서 클로저 구문 완전 지원 (현재 Known Limitation)
- [ ] 표준 라이브러리 고차 함수 패턴: `map`, `filter`, `fold`, `for_each`
- [ ] 클로저 타입 추론 강화 (컨텍스트 기반 파라미터 타입 추론)
- [ ] 클로저 E2E 테스트 10개
- **난이도**: 중 | **모델**: Opus 직접
...
- [ ] 제네릭 + 트레이트 조합 엣지 케이스 15개
- [ ] 클로저 + 소유권 상호작용 10개
- [ ] 에러 처리 (`?`, `!`) 체이닝 10개
- [ ] 멀티 모듈 프로젝트 E2E 10개
- [ ] 문자열 보간 + 포매팅 엣지 케이스 10개
- [ ] 이터레이터/고차 함수 E2E 10개 (Stage 4 이후)
- **난이도**: 하 | **모델**: Sonnet 위임
