...
> **상태**: 🔄 진행 중 (Stage 1 - 기능 패리티 강화 완료, 예제 61% 통과 (72/117), 기본값 전환 대기)
...
### 1단계 - 파일 시스템 내구성 (ACID 필수) 🔴 차단

> fsync 없이는 ACID의 D(Durability)를 보장할 수 없음. DB 엔진의 가장 기본 요구사항.

- [ ] **`fsync`/`fdatasync` FFI 추가** - `std/file.vais`에 `F fsync(fd: i64) -> i64` extern 선언, POSIX `fsync()` 바인딩
- [ ] **`file_sync()` 고수준 API** - File 구조체에 `sync()` 메서드 추가, 쓰기 후 디스크 플러시 보장
- [ ] **디렉토리 동기화** - `dir_sync()` 추가 (메타데이터 내구성 보장, WAL 파일명 변경 후 필수)
- [ ] **테스트** - fsync 호출 후 크래시 시뮬레이션, 데이터 무결성 검증

### 2단계 - 메모리 매핑 (페이지 캐싱 필수) 🔴 차단
...
- [ ] **`flock` FFI 추가** - `LOCK_SH`/`LOCK_EX`/`LOCK_NB`/`LOCK_UN` 바인딩
- [ ] **`fcntl` 레코드 잠금** - 바이트 범위 잠금 (영역별 동시 접근 허용)
- [ ] **`FileLock` 고수준 API** - RAII 기반 잠금, 스코프 종료 시 자동 해제
- [ ] **테스트** - 다중 프로세스 동시 잠금 시나리오, 교착 상태 방지 검증

### 4단계 - 할당자 상태 변이 수정 🟡 높음

> 현재 BumpAllocator/PoolAllocator/FreeListAllocator가 구조체를 값으로 전달하여 상태가 실제 갱신되지 않음.

- [ ] **할당자 포인터 기반 상태 관리** - 구조체 내부 상태를 포인터를 통해 변이하도록 수정
- [ ] **BumpAllocator 수정** - offset 증가가 실제 반영되도록 `alloc()` 수정
- [ ] **PoolAllocator 수정** - free_list 갱신이 실제 반영되도록 수정
- [ ] **FreeListAllocator 수정** - 할당/해제 시 free_list 연결 리스트 정상 갱신
- [ ] **StackAllocator 수정** - top 포인터 갱신 반영
- [ ] **테스트** - 연속 할당 후 주소 겹침 없음 검증, 해제 후 재할당 검증

### 5단계 - 문자열 키 HashMap & 소유 문자열 🟡 높음

> SQL 테이블명, 컬럼명, 인덱스명 등 문자열 키 기반 조회 필수. 현재 HashMap은 i64 키만 지원.

- [ ] **문자열 해시 함수** - FNV-1a 또는 SipHash 기반 `str_hash()` 구현
- [ ] **문자열 비교 함수** - `str_eq()` 바이트 단위 비교 (HashMap 키 비교용)
- [ ] **`StringMap<V>` 타입** - 문자열 키 전용 HashMap, 해시+비교 내장
- [ ] **소유 문자열 (OwnedString)** - 길이 추적 + 힙 할당 + 자동 해제, `str`과 변환 가능
- [ ] **테스트** - 문자열 키 삽입/조회/삭제, 충돌 해결, OwnedString 생명주기
...
- [ ] **`?` 연산자 코드젠 완성** - Result/Option 체이닝이 실제 LLVM IR로 정상 생성
- [ ] **에러 타입 변환** - `From` 트레이트 기반 자동 에러 타입 업캐스팅
- [ ] **에러 컨텍스트 첨부** - `.context("msg")` 메서드로 에러 원인 추적
- [ ] **테스트** - 다층 함수 호출에서 `?` 체이닝, 에러 타입 변환 검증

### 검증 기준

| 단계 | 검증 항목 | 우선순위 |
|------|----------|----------|
| 1단계 | fsync 후 데이터 영속 확인, File.sync() E2E | 🔴 차단 |
| 2단계 | 1GB 파일 mmap 읽기/쓰기 E2E | 🔴 차단 |
| 3단계 | 2개 프로세스 동시 flock 검증 | 🟡 높음 |
| 4단계 | 1000회 연속 alloc 후 주소 유일성 검증 | 🟡 높음 |
| 5단계 | 10만 개 문자열 키 HashMap 삽입/조회 성능 | 🟡 높음 |
| 6단계 | 디렉토리 CRUD + rename 원자성 | 🟢 중간 |
| 7단계 | 바이너리 라운드트립 100% 정합성 | 🟢 중간 |
| 8단계 | `?` 체이닝 3단계 이상 에러 전파 | 🟢 중간 |
