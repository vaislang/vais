...
- [ ] **FP9** (선택) codegen에 다중 인자 / 지역변수(let in body) 확장. 또는 인터프리터-codegen 통합.
...
- [ ] **FP10e** struct codegen + 동적 List(push/len) — 진짜 self-compile의 큰 남은 조각(months급).
- [ ] **FP10f** 함수+명령형본문 통합(함수 안 while/let mut/if) — nl 컴파일러 자신의 함수 형태.
