...
2. [x] **NV-C1 자체 컴파일러 front 계약 고정**: native subset(Int 함수/let/return/if/while/plain call + print/putchar IO + simple struct + List push/len/index)을 `scripts/vaisc` preflight로 고정하고, unsupported 문법은 `help:` 진단으로 거절.
...
- New Vais front 계약 smoke `scripts/test-vaisc-front.sh` **OK**:
  accepted scalar subset exit 42, print interpolation/putchar stdout, simple struct, List push/len/index 검증,
...
- New Vais parity manifest gate `scripts/test-vaisc-parity.sh` **OK**:
  `native-supported=26`, `bootstrap-only=8`, `tracked=0`.
...
5. **자체 컴파일러 mainline** — `NV-C0` 제품 경계, `NV-C1` front 계약,
   ...
   slice를 `examples/c4.nl`/`examples/e75_list_push.nl`로 native-supported에 편입했다.
