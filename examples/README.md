...
ingest/query/report/ingest-dir/reindex/rank/search/msearch/similar/top/phrase/why/docs/remove/stats
...
document's stored source; top aggregates the index-wide most frequent
terms; phrase ranks exact word sequences over stored sources; why breaks
a document's score into per-term contributions), a deterministic
no-argument self-test, and a release archive,
...
arithmetic stages, `tostring`/`tonumber`, `recurse`, `paths`/`leaf_paths`,
`del(...)`, `keys_unsorted`,
