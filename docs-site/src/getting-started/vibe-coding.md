...
Step 19 이후 새 코드는 retired single-char form을 쓰지 않습니다. `F/S/E/EN/EL/M/R/T/U/P/W/X`는 제거되었고 아래 multi-char form이 표준입니다.
...
아직 single-char인 core control form은 그대로 남아 있습니다: `I` if, `L` loop, `A` async, `Y` await, `D` defer, `G` global, `O` union.
...
핵심: `LF` range loop, `I/else I/else` if-else-if 체인, 문자열 보간 `{i}`.
...
- **제거된 키워드 주의**: `F/S/E/EN/EL/M/R/T/U/P/W/X`, `weak`, `clone`, `consume`은 제거됨. AI가 이들을 사용하려 하면 canonical form으로 바꾸거나 거부.
- **실험 기능 금지**: `lazy`, `force`, HKT, `impl Trait` (dyn이 아닌 것)은 아직 제한적. 프로덕션 코드에서 제외.
- **생태계 import**: VaisDB는 `use vaisdb::{Database}`, vais-server는 `use core/app::{App}` 형식.
