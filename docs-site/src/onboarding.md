...
#### 학습 목표
- Vais의 canonical keyword와 남아 있는 single-char control form 이해
- 변수 바인딩 및 기본 타입 학습
- 첫 번째 함수 작성
...
| 키워드 | 의미 | 사용 예 |
|--------|------|--------|
| `fn` | Function (함수) | `fn add(a: i64, b: i64) -> i64` |
| `I` | If (조건문) | `I x > 0 { ... }` |
| `else` | Else (선택적) | `else { ... }` |
| `L` | Loop (반복문) | `L { ... }` |
| `LF` | Range loop | `LF i: 0..10 { ... }` |
| `match` | Match (패턴 매칭) | `match value { pattern => ... }` |
| `return` | Return (반환) | `return x + y` |
| `B` | Break (루프 탈출) | `B` |
| `struct` | Struct (구조체) | `struct Point { x: i64, y: i64 }` |
| `enum` | Enum (열거형) | `enum Result { Ok, Err }` |
| `trait` | Trait (특성) | `trait Comparable { ... }` |
...
fn is_positive(n: i64) -> bool = n > 0 ? true : false

fn main() -> i64 = max(42, 10)  # 42 반환
```
...
fn handle_result(result: Result<i64, str>) -> i64 {
    match result {
...
fn get_first<T>(arr: *type) -> T = arr[0]
...
impl impl User {
    fn new(id: i64, name: str, email: str, age: i64) -> Self {
...
# 리스트의 합 계산 (포인터 사용)
fn sum_array(arr: *i64, len: i64, idx: i64) -> i64 {
...
# 클로저를 함수 인자로 전달
fn apply_twice<T>(f: |type| -> T, x: T) -> T {
    f(f(x))
}
...
```vais
# 리스트 처리 (포인터 배열)
fn process_list(arr: *i64, len: i64) -> i64 {
    idx := mut 0
    sum := mut 0
...
# 필터링 패턴
fn count_positives(arr: *i64, len: i64, idx: i64) -> i64 {
    idx >= len ? 0 : (arr[idx] > 0 ? 1 : 0) + @(arr, len, idx + 1)
}

# 매핑 패턴
fn double_value(x: i64) -> i64 = x * 2
```
...
fn print_all<T: Drawable>(items: *type, len: i64) {
    idx := mut 0
    L {
        I idx >= len {
            B
...
fn serialize<T: Comparable + Drawable>(item: T) -> str {
    item.draw()
}
```
...
type MathOps {
    ...
    fn multiply(self, scale: i64) -> Self
}
...
fn process_vectors<T: MathOps>(v1: T, v2: T, scale: i64) -> T {
    result := v1.add(v2)
...
impl impl Calculator {
    fn new() -> Self {
        Self { last_result: 0 }
    }

    fn execute(mut self, op: Operation, a: i64, b: i64) -> i64 {
        result := match op {
            Operation::Add => a + b,
    ...
    fn get_last(self) -> i64 = self.last_result
}
...
impl impl TodoList {
    ...
    fn add_task(mut self, id: i64, title: str) {
    ...
    fn complete_task(mut self, id: i64) {
        idx := mut 0
        L {
            I idx >= self.count {
                B
...
# 테스트 헬퍼 함수
fn assert_equal(expected: i64, actual: i64, msg: str) -> bool {
    I expected == actual {
        puts("✓ PASS: " + msg)
        true
    } else {
...
fn map<T, use>(maybe: Maybe<T>, f: |type| -> use) -> Maybe<use> {
    match maybe {
...
impl impl ConfigBuilder {
    ...
    fn with_port(mut self, port: i64) -> Self {
        self.port = port
        self
    }
...
```vais
# 비동기 함수 정의
fn async fetch_data(url: str) -> str {
    # 실제 HTTP 요청 수행
    "response data"
}

# await를 사용한 비동기 대기 (Y 축약 사용 가능)
fn async process_data() -> i64 {
...
# 제네릭 타입 별칭
type List<T> = *type
type Dict<K, V> = struct { keys: *K, values: *V }
...
# 조건부 컴파일
#[cfg(target = "x86_64")]
fn cpu_optimized() -> i64 = 42

#[cfg(target = "wasm")]
fn wasm_version() -> i64 = 24
```
...
impl impl HttpServer {
    fn new(host: str, port: i64) -> Self {
        Self {
            host: host,
            port: port,
            handlers: malloc(100)
    ...
    fn register_route(mut self, path: str, handler: str) {
        # 라우트 등록
    }
...
```vais
# 좋은 예
fn calculate_average(values: *i64, len: i64) -> i64 =
    sum_array(values, len) / len

# 나쁜 예
fn calculate_average(values:*i64,len:i64)->i64=sum_array(values,len)/len
```
