...
| `fn name() { }` | 함수 정의 | `fn add(a, b) { return a + b }` |
| `A fn name() { }` | 비동기 함수 | `A fn load() { }` |
| `struct Name { }` | 구조체 정의 | `struct User { name: String }` |
| `I cond { }` | if 조건 | `I x > 0 { return true }` |
| `else I cond { }` | else if | `else I x == 0 { }` |
| `else { }` | else | `else { return false }` |
| `return value` | 반환 | `return result` |
| `#[server]` | 서버 전용 속성 | `#[server] A fn load()` |
