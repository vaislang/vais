...
```vais
# 좋은 예
fn calculate_sum(numbers: [i64; 10]) -> i64 { }
fn is_prime(n: i64) -> bool { }
fn get_user_by_id(id: i64) -> Option<User> { }
fn parse_json(text: str) -> Result<Json> { }
fn validate_email(email: str) -> bool { }

# 나쁜 예
fn CalcSum(numbers: [i64; 10]) -> i64 { }      # PascalCase 사용 금지
fn isprime(n: i64) -> bool { }                 # 약자 사용
fn getUserById(id: i64) -> Option<User> { }    # camelCase 사용 금지
fn PARSE_JSON(text: str) -> Result<Json> { }   # SCREAMING_SNAKE_CASE 함수명 사용 금지
```
...
# 나쁜 예
struct user { }                # snake_case 사용 금지
struct userProfile { }         # camelCase 사용 금지
struct USER_DATA { }           # SCREAMING_SNAKE_CASE 사용 금지
```
...
# 나쁜 예
trait drawable { }        # snake_case 사용 금지
trait draw_able { }       # snake_case 사용 금지
```
...
# 나쁜 예
enum color { }           # snake_case 사용 금지
enum COLOR { }           # SCREAMING_SNAKE_CASE 사용 금지
enum http_method { }     # snake_case 사용 금지
```
...
```vais
# 좋은 예: 왜 이렇게 하는지 설명
fn find_user_efficiently(id: i64) -> Option<User> {
...
```vais
# 좋은 예
fn first_function() -> i64 {
    42
}

fn second_function() -> str {
    "hello"
}
...
fn query_user(id: i64) -> Result<User> {
    I id <= 0 {
        return Result.Err(DatabaseError.QueryFailed)
    }
    # ...
}

# 나쁜 예: 문자열로 에러 표현
fn query_user_bad(id: i64) -> Result<User> {
    I id <= 0 {
        return Result.Err("bad id")
    }
    # ...
}
```
...
```vais
# 좋은 예: 명확한 타입
fn process_data(items: [i64; 10]) -> i64 {
    sum: i64 := mut 0
    # ...
}
...
```vais
# 좋은 예: 참조 사용
fn process_large_data(data: &[i64; 1000]) {
    # data 사용
}

# 나쁜 예: 복사 발생
fn process_large_data_bad(data: [i64; 1000]) {
    # 전체 배열이 복사됨
}
```
