...
fn background_task(id: i64) -> Future<()> {
    sleep(1000)
...
fn worker(barrier: Barrier, id: i64) -> Future<()> {
...
fn limited_task(sem: Semaphore, id: i64) -> Future<()> {
    Y sem.acquire()
...
fn task(wg: WaitGroup, id: i64) -> Future<()> {
    sleep(id * 100)
    puts("Task {id} done")
    wg.done()
}
...
fn generate_numbers() -> AsyncStream<i64> {
...
fn handle_connection(stream: AsyncTcpStream) -> Future<()> {
    buffer := [0u8; 1024]
...
fn fetch_data() -> Future<Result<str, Error>> {
    # May fail
    async_http_get("https://api.example.com/data")
}
