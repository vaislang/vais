...
```vais
fn parse_and_process(data: str) -> Result<i64, str> {
    # parse가 실패하면 즉시 Err 반환
    num := parse_i64(data)?

    # 검증 실패 시 커스텀 에러
    I num < 0 {
        return Result.Err("음수는 허용되지 않습니다")
    }
...
fn safe_divide(a: i64, b: i64) -> Result<i64, str> {
    I b == 0 {
        return Result.Err("0으로 나눌 수 없습니다")
    }
    Result.Ok(a / b)
}
...
```vais
fn get_first_char(s: str) -> Option<i64> {
    I strlen(s) == 0 {
        return Option.None
    }
...
fn process_string(s: str) -> Option<i64> {
    # Option에서도 ? 연산자 사용 가능
...
fn read_file(path: str) -> Result<str, FileError> {
    I !file_exists(path) {
        return Result.Err(FileError.NotFound(path))
    }

    I !can_read(path) {
        return Result.Err(FileError.PermissionDenied(path))
    }
...
fn parse_positive_number(s: str) -> Result<i64, ParseError> {
    # 기본 파싱
    match parse_i64(s) {
        Result.Ok(num) => {
            I num < 0 {
                return Result.Err(ParseError.OutOfRange(num))
            }
            Result.Ok(num)
        },
        Result.Err(_) => {
...
fn load_and_parse_config(path: str) -> Result<i64, str> {
...
```vais
fn process_data(input: str) -> Result<i64, str> {
    input
        |> validate_input
        |> parse_number
        |> transform_value
...
fn validate_input(s: str) -> Result<str, str> {
    I strlen(s) == 0 {
        return Result.Err("빈 입력")
    }
    Result.Ok(s)
}
...
fn transform_value(n: i64) -> Result<i64, str> {
    Result.Ok(n * 2)
}
```
...
```vais
# ✅ 좋은 예: 라이브러리 함수
fn lib_parse(data: str) -> Result<i64, str> {
    I strlen(data) == 0 {
        return Result.Err("빈 데이터")
    }
    parse_i64(data)
}
```
...
fn open_file(path: str) -> Result<File, str> {
    I !file_exists(path) {
        return Result.Err("파일을 찾을 수 없습니다: " + path)
    }
    # ...
}
```
...
# ✅ 올바른 방법
fn good_example() -> Result<i64, str> {
    might_fail()?
    Result.Ok(0)
}
```
...
fn read_config_file(path: str) -> Result<str, FileError> {
    # 파일 존재 확인
    I !file_exists(path) {
        return Result.Err(FileError.NotFound(path))
    }
    ...
    # 파일 열기
    file := match open(path) {
        Result.Ok(f) => f,
        Result.Err(e) => {
            return Result.Err(FileError.ReadError(e))
        }
    }

    # 내용 읽기
    content := match read_to_string(file) {
        Result.Ok(c) => c,
        Result.Err(e) => {
            close(file)
            return Result.Err(FileError.ReadError(e))
...
fn fetch_data(url: str) -> Result<str, HttpError> {
    # 연결 시도
    conn := match connect(url) {
        Result.Ok(c) => c,
        Result.Err(e) => {
            return Result.Err(HttpError.ConnectionFailed(e))
        }
    }

    # 요청 전송 (타임아웃 5초)
    response := match send_request(conn, 5000) {
        Result.Ok(r) => r,
        Result.Err("timeout") => {
            return Result.Err(HttpError.Timeout)
        },
        Result.Err(e) => {
            return Result.Err(HttpError.ConnectionFailed(e))
        }
    ...
    I status != 200 {
        return Result.Err(HttpError.InvalidResponse(status))
    }
...
fn parse_user_json(json: str) -> Result<User, JsonError> {
    # JSON 파싱
    obj := match parse_json(json) {
        Result.Ok(o) => o,
        Result.Err(msg) => {
            line := find_error_line(json, msg)
            return Result.Err(JsonError.SyntaxError(line, msg))
        }
    }

    # 필드 추출
    name := match get_string_field(obj, "name") {
        Result.Ok(n) => n,
        Result.Err(_) => {
            return Result.Err(JsonError.TypeError("name 필드가 없거나 문자열이 아닙니다"))
        }
    }

    age := match get_i64_field(obj, "age") {
        Result.Ok(a) => a,
        Result.Err(_) => {
            return Result.Err(JsonError.TypeError("age 필드가 없거나 숫자가 아닙니다"))
        }
    }
