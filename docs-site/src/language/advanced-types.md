...
# 다중 트레이트 별칭
type StringLike = Display + Clone + Hash
```
...
struct MyString { data: str }
impl MyString: Display {
    fn show(self) -> i64 {
        puts(self.data)
        0
    }
...
fn process<T: StringLike>(val: T) -> i64 {
    val.show()
    cloned := val.clone()
    hash := val.hash()
    hash
...
fn use_nested<T: Level2>(x: T) -> i64 {
    x.base_fn()
}
