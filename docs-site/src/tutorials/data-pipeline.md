...
fn store_i64(buf: i64, offset: i64, value: i64) {
...
fn load_i64_at(buf: i64, offset: i64) -> i64 {
    result := mut 0
    L i:0..8 {
...
```vais
# 레코드 하나 읽기
fn read_record(buf: i64, index: i64) -> Record {
...
fn apply_curve(score: i64) -> i64 {
    curved := score + score / 10
    I curved > 100 { return 100 }
    curved
}

# 필터: 최소 점수 이상만 통과
fn passes_filter(score: i64, min_score: i64) -> i64 {
    I score >= min_score { return 1 }
    0
}
```
...
```vais
fn compute_stats(buf: i64, count: i64) -> Stats {
    total := mut 0
    max_v := mut 0
    min_v := mut 999999
...
```vais
fn compute_filtered_stats(buf: i64, count: i64, min_score: i64) -> Stats {
    total := mut 0
    max_v := mut 0
    min_v := mut 999999
    passed := mut 0
...
```vais
# 단일 레코드 처리
fn process_score(score: i64) -> i64 {
    score |> apply_curve |> |s| I s > 100 { 100 } else { s }
}
```
