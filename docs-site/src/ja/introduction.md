...
これは Vais の LLVM IR emission と、他 toolchain の full binary compilation
を比較する scoped benchmark claim です。古い large-scale throughput snapshot
は historical evidence であり、current data として引用する前に rerun が必要です。
