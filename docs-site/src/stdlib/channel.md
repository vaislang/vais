...
fn worker(ch_ptr: i64, worker_id: i64) -> i64 {
    receiver := load_typed(ch_ptr)
...
fn fast_producer(ch_ptr: i64) -> i64 {
    sender := load_typed(ch_ptr)
    i := 0
    L i < 1000 {
...
fn slow_consumer(ch_ptr: i64) -> i64 {
    receiver := load_typed(ch_ptr)
    i := 0
    L i < 1000 {
        value := receiver.recv()
...
```vais
fn recv_with_timeout(ch: &Receiver<i64>, ms: i64) -> i64? {
    timer := channel_new(1)
