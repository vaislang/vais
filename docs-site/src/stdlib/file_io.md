...
fn read_lines(path: i64) -> Vec<i64> {
    lines := Vec::new()
    f := File::open_read(path)
...
fn save_data(path: i64, data: i64, size: i64) -> i64 {
...
fn main() -> i64 {
    buffer := malloc(100)
    # buffer에 데이터 채우기...
    return save_data("output.bin", buffer, 100)
}
```
...
fn append_log(path: i64, message: i64) -> i64 {
    f := File::open_append(path)
    I !f.is_valid() { return 1 }
...
```vais
# 대용량 파일 처리
fn process_large_file(path: i64) -> i64 {
    f := File::open_read(path)
    L f.is_valid() {
...
fn open_checked(path: i64) -> Result<File, FileError> {
    f := File::open_read(path)
    I !f.is_valid() {
