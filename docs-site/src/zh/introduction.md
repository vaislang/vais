...
- **自托管 workbench** — 50,000+ 行 Vais compiler source，用于 bootstrap 和 conformance work
- **6.3ms average single-file `--emit-ir`** in the 2026-05-13 scoped compile-speed benchmark
...
该数据比较的是 Vais LLVM IR emission 与其他 toolchain 的 full binary
compilation。旧的大规模 throughput snapshot 是 historical evidence，引用为当前
数据前需要重新运行。
