...
- **自托管** - 自托管编译器(bootstrap 已实现) — v0.9.0、51,190 行、58 个 .vais 文件
...
- **自托管编译器** — bootstrap 已实现，v0.9.0，51,190 行，58 个 .vais 文件
- **800K 行/秒** 的编译速度 (50K 行 → 63ms)
- **12,000+ 个测试** 覆盖所有组件，2,592 个 E2E 测试
...
**自托管:** 自托管编译器(bootstrap 已实现) — v0.9.0、51,190 行、58 个 .vais 文件可编译为原生二进制。
...
```bash
cargo build --release
cargo test                                     # 运行所有 12,000+ 测试
cargo test -p vaisc --test e2e_tests           # 运行 2,592 个 E2E 测试
