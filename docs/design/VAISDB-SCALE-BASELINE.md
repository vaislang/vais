...
## P4 결과 (2026-07-26k) — 제품 래더 종결

vaisdb-scale 게이트 신설(scripts/vaisdb-scale-gate.sh): 결정적 합성
코퍼스(60문서×203고유 term = 포스팅 12,180)로 ingest-dir(예산 20s, 실측
~0.3s)과 rank 읽기 경로(예산 5s)를 vaisbench 예산 검사 + 정확성 스팟
(top score/stats). gates.tasks 16번째 태스크로 래더 체인(fmt+perf+
vaisdb-scale+release) 편입 — 스케일 회귀가 래더에서 예산 초과로 표면화.

**P1(Map 4096)~P2(디스크-우선)~P3(견고성)~P4(스케일 게이트) 제품 래더
종결.** 베이스라인의 병목·즉사 시나리오가 전부 해소·게이트화됐다. 잔여
후보(초대형 문서 분할 ingest, docs 4095 상한, 주석 스캐너)는 수요 시.
