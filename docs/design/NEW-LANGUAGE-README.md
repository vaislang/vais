...
3. **front 계약 유지** — native subset은 Int 함수/let/return/if/while/plain call, print/putchar, simple struct, payload-free enum/match, small Int-coded payload enum/match, List push/len/index/sum slice이며 unsupported 문법은 `help:` 진단으로 실패한다.
