...
- Current std/http_client runtime smoke compiles, links, and runs a real
  ...
  loopback absolute `302 Location: http://127.0.0.1:<port>/final` plus
  root-relative `302 Location: /final` redirects.
...
- "std/http_client HTTPS redirects, scheme-relative redirects, path-relative
  dot-segment resolution, method-preservation nuances, keep-alive pooling,
