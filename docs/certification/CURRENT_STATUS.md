...
| vais-server auth JWT token/verify | role-free deterministic access/refresh token encode/generation, token pair generation, role-free plus role-bearing verification/decode success, and empty/malformed/bad-signature/expired/issuer-mismatch errors covered by current `20/20` smoke |
