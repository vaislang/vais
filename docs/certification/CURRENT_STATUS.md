...
| VaisDB public SQL WAL recovery proof | Descriptor-backed `books(id INT, title TEXT)` values-only INSERT now has bounded WAL redo evidence: the public smoke replays public INSERT WAL from the post-CREATE LSN over a pre-insert heap page image and verifies recovered rows through public SELECT after reopen. Automatic recovery-on-open, descriptor CREATE recovery, public UPDATE/DELETE recovery, index/constraint recovery, planner execution, and broad SQL recovery remain follow-up |
