...
| vais-server runtime smoke | `25/25`; includes the fixed DB-backed `/db/books` route smoke that performs trusted descriptor-backed books setup, opens a real EmbeddedDatabase handle in the route helper, runs `SELECT * FROM books`, verifies the seeded rows, frees result text, closes the handle, and returns the fixed JSON envelope through `Context.json()` |
...
| vais-web packages | `tests=3296/3296` |
