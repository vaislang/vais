...
|---|---|---|---|---|
| broad implicit coercion (the user-facing surfaces verified by runtime fixtures) | **REMOVAL TARGET (A4-runtime-silent / A4-late-codegen-silent)** — see "Removal queue (A4)" below for the 12-entry verified inventory. | **high — silent acceptance** (proven by empirical A4 fixtures producing wrong runtime exit codes or late codegen failures before strict-default fixes). | AI-native frame requires every conversion to be explicit. No promotion path. | A4 removal queue. Not promoted under any condition. |
...
| `?` error propagation outside A2-01/A2-02 | bounded A2 subset landed for Core-typed Result/Option propagation; broad/wrong-receiver/cleanup semantics remain excluded | medium — out-of-subset `?` can imply cleanup or ownership behavior the compiler does not certify | A2 certifies value/error propagation only, including the cross-module vaisdb baseline path. Cleanup-on-early-return, ownership transfer, and broad generic conversion stay outside Core proof. | New Result/Option design plus positive/negative fixtures for the wider behavior. |
| trait objects / vtables outside A2-03 | bounded A2 subset landed for declared traits with visible impls and dyn dispatch; broad casts/ABI/lifetime/ownership behavior remain excluded | high if generalized — invalid non-implementers now reject, but broad dyn construction and ABI widening are not certified | A2-03 certifies only the narrow dispatch predicate in `A2_SUBSETS.md`. | Dedicated design doc, call dispatch invariant, and fixtures for the wider dyn surface. |
...
| closures beyond A2-04 inline/no-escape cases | partial (~369 LOC, 2–3 simple-case e2e fixtures) | high — capture analysis exists; escaping closures are rejected by A4-15 and first-class closure storage is not certified | A2-04 certifies only inline/synchronous no-escape closures. | Closure parameter/return type invariant and fixtures for stored/returned closures. |
| function values beyond A2-05 named-function parameters | bounded A2 subset landed for Core-typed `fn(...) -> ...` parameters with named-function arguments | high if generalized — stored callbacks, arbitrary first-class function values, and closure-to-fn-pointer coercions are not certified | A2-05 certifies only bounded named-function pointer parameters. | New RFC and parser/type/codegen tests for the wider function-value surface. |
| `drop` / auto-free semantics | partial, narrow gates only: Row payload Drop and AUTODROP-marked standalone `SqlValue.drop` are certified in their existing VaisDB row payload gates; Token/AST/EmbeddedResult automatic Drop and broad Result cleanup-on-return remain unpromoted. | high if generalized — automatic cleanup can double-free shallow copies or skip early-return/resource paths unless the ownership bit/proven helper boundary is explicit. | W1-B decision: Token, parser AST, and public result cleanup stay manual (`free_token_vec_owned_text()`, `Statement.free_parser_owned_text()`, `EmbeddedResult.free_owned_text()`). Broad RAII/destructor lowering belongs to Step 16/I-5. | Per-surface ownership/destructor design, positive/negative runtime fixtures, and same-class audit before any new automatic Drop promotion. |
...
### T-499 W1-B automatic Drop nonclaim fixture

`compiler/tests/empirical/Rejected/R-04_automatic_drop_nonclaims` guards the
W1-B decision above. It requires the manual Token, parser AST, and
`EmbeddedResult` cleanup helpers, requires this document's nonclaim wording, and
drifts if an unpromoted Token/Statement/EmbeddedResult Drop implementation
appears.

### T-509 W1-C unsupported-feature diagnostic wording refresh

T-509 refreshes rejected-surface diagnostic evidence without promoting any
excluded feature. `crates/vais-types/src/checker_expr/control_flow.rs` now
reports A4-06 integer-truthy control-flow predicates in user order as expected
`bool`, found `i64`. `crates/vaisc/tests/error_snapshot_tests.rs` pins two
representative rejected-surface diagnostics:

- `integer_truthy_condition_rejected.snap`: A4-06 integer truthy condition.
- `escape_closure_rejected.snap`: A4-15 escape closure capture.

These snapshots prove deterministic wording for already-rejected surfaces only.
They do not promote integer truthy predicates, escape closures, broad closure
storage, release, deployment, or production behavior.

### T-528 W1-E empirical refresh

T-528 refreshed the W1-E empirical language-surface gates on 2026-05-25:

- `bash scripts/check-empirical.sh A1` PASS: 3 pass / 0 drift / 0 broken /
  0 skipped.
...
This refresh keeps the generated A2/A4 inventories as evidence boundaries. It
does not promote, demote, or widen any excluded feature.

<!-- inventory:auto-start -->

<!-- Generated by scripts/render-excluded-features.py from master-plan.toml v312. -->
<!-- DO NOT HAND-EDIT this section. Edit master-plan.toml then run the script. -->

## A2 certified subsets

A2-01 through A2-05 are landed certified subsets. A2-06 affine was reclassified to Controlled. New A2 candidates must be added only after an explicit roadmap revision with formal subset predicates and fixtures.

Landed A2 subsets tracked by this plan revision:
...
## Removal queue (A4) — 12 verified entries
...
**Status**: v2 retro-validation complete for the active A4 fixture set: all 14 empirical A4 runners pass as of v112 (2026-05-12); 11 strict-default, 1 specified-safe, 2 Controlled/design fixtures retained for regression coverage; still_silent_count is 0. v1 discovery began 2026-05-03. v17 reclassified A4-05/A4-07 to Controlled; v47 restored A4-03 to A4 after user-code evidence; v99 closed A4-13; v100 specified A4-14; v103 closed A4-03 by default.

Codemod dependency: Order step 2.

### A4-runtime-silent (10 entries) — historical discovery accepted; current status is listed

| ID | Surface | Site | Probe | Expected | Historical actual | Current status/evidence |
|---|---|---|---|---|---|---|
| A4-01 | Unit ↔ i64 (void return as i64) | `unification.rs:361` | x: i64 = void_fn() | 0 | 96 | strict_default_landed — tests/empirical/A4/A4-01_unit_i64/run.sh PASS — 'vaisc check rejects probe with E001 (silent surface removed; default-mode strict)'. Env opt-out: VAIS_REJECT_A4_01=0. |
| A4-02 | Pointer<T> ↔ i64 | `unification.rs:410` | take_i64(p) where p: *i64 | 42 | 184 | strict_default_landed — tests/empirical/A4/A4-02_pointer_i64/run.sh PASS — strict default. Env opt-out: VAIS_REJECT_A4_02=0. |
| A4-04 | Pointer<T> ↔ Slice<T>/SliceMut<T> (Phase 162) | `unification.rs:417` | take_slice(p as *u8) | 4 | 72 | strict_default_landed — tests/empirical/A4/A4-04_pointer_slice/run.sh PASS — strict default. Env opt-out: VAIS_REJECT_A4_04=0. |
| A4-06 | Integer truthy (4 sites: else-if, ternary, if, while) | `control_flow.rs:188,243,273,396` | I x { 100 } EL { 200 } where x: i64 | design violation (no `!= 0` token) | 100 — runtime correct, design violated | strict_default_landed — tests/empirical/A4/A4-06_integer_truthy/run.sh PASS — positive (explicit `!= 0`) exits 100; negative (integer-as-truthy) rejected. Env opt-out: VAIS_REJECT_A4_06=0. |
| A4-10 | Struct partial-init (silent acceptance of missing fields) | `vais-types struct constructor checker` | S Person { name: str, age: i64 }; let p: Person = Person { name: "x" } | E001 missing fields | (historical) silent accept; struct created with garbage age field | strict_default_landed — tests/empirical/A4/A4-10_struct_partial_init/run.sh PASS — 'vaisc check rejects probe with missing fields: age (silent surface removed; default-mode strict)'. Discovered via STEP7 F-15. |
| A4-11 | Try operator (?) in function returning non-Result/Option | `vais-types try expression checker` | F take(x: i64) -> i64 { x? } | E001 expected Result<_,_> or Option<_> | (historical) silent accept; ? lowered as identity | strict_default_landed — tests/empirical/A4/A4-11_try_in_non_result/run.sh PASS — 'vaisc check rejects probe with E001 expected Result<_,_> or Option<_>'. |
| A4-13 | Box<T> ↔ T auto-unwrap (call site) | `vais-types/checker_expr/calls.rs (Box<T> arg accepted where T expected)` | F take_i64(x: i64) -> i64 { R x }; F take_box(b: Box<i64>) -> i64 { R take_i64(b) } | E001 Type mismatch (Box<i64> is not i64) | historical: OK No errors found (silent accept); current default rejects with E001 | strict_default_landed — tests/empirical/A4/A4-13_box_t_auto_unwrap/run.sh PASS — default mode rejects direct Box<i64> where i64 is expected with E001; VAIS_REJECT_A4_13=0 preserves legacy silent accept for drift investigation. |
| A4-14 | Vec<T> ↔ &[T] call-site coercion (.len() path) — historically runtime corruption | `vais-types/inference/unification.rs (Vec ↔ Slice arm)` | fn len_of(s: &[i64]) -> i64 { s.len() as i64 }; main: v: Vec<i64> = [1,2,3]; len_of(&v); expects 3 | exit 3 (Vec length) | historical: exit ≠ 3 (memory-load corruption — fat-pointer Vec layout misread as Slice header); current: exit 3 after materialized slice lowering | specified_safe_landed — tests/empirical/A4/A4-14_vec_slice_len/run.sh PASS — &Vec<i64> is materialized as &[i64] and runtime returns 3. VAIS_REJECT_A4_14=1 forces strict migration audit. |
| A4-15 | Escape closure: closure returned from function loses captured environment | `vais-types/checker_expr/stmts.rs Stmt::Return + vais-types/checker_fn.rs check_function trailing-expression branch (TC detector)` | fn make_adder(x: i64) -> |i64| -> i64 { |n| n + x } | E001 'expected named function pointer or capture-free closure, found escape closure capturing N variable(s)' | rejected at TC layer (default-strict, opt-out via VAIS_REJECT_A4_15=0) | strict_default_landed — tests/empirical/A4/A4-15_escape_closure_capture/run.sh PASS as 'escape closure rejected by TC detector with E001 + A4-15 marker'. baseline impact 0 (std=0/82, LIVING_SPEC=0/117, vaisdb 11/282 = pre-detector baseline). |
| A4-03 | Auto-deref &T ↔ T (type-check silent path) | `vais-types/inference/unification.rs:754` | m: HashMap<u64, u64> = HashMap.new(); m.get_opt(&key) where key: u64 | E001 Type mismatch (&u64 is not u64) at TC layer | TC accepts; codegen lowers as i64 instead of ptr; clang catches loud: '%t6' defined with type 'ptr' but expected 'i64' at HashMap_get_opt$u64_u64 | strict_default_landed — tests/empirical/A4/A4-03_auto_deref/run.sh PASS — default mode rejects implicit &T -> T with E001; VAIS_REJECT_A4_03=0 preserves the legacy accept path only for drift investigation; explicit *r positive exits 42. Full integrity OK. |

### A4-late-codegen-silent (2 entries) — historical discovery reached codegen/linker; current status is listed

| ID | Surface | Site | Probe | Historical failure | Current status/evidence |
|---|---|---|---|---|---|
| A4-08 | Vec<T> ↔ &T permissive | `unification.rs:384` | take_str_ref(v) where v: Vec<i64> | clang IR mismatch ({ptr,i64} vs ptr) — late codegen failure | strict_default_landed — tests/empirical/A4/A4-08_vec_str_permissive/run.sh PASS — strict default at type-check before reaching late codegen. Env opt-out: VAIS_REJECT_A4_08=0. |
| A4-09 | Lifetime ref erasure (function definition) | `unification.rs:450` | F take_lifetime_ref<'a>(r: &'a i64) -> i64 { ... } and call with plain &i64 | linker undefined symbol _take_lifetime_ref | strict_default_landed — tests/empirical/A4/A4-09_lifetime_ref_erasure/run.sh PASS — strict default at type-check before reaching linker. Env opt-out: VAIS_REJECT_A4_09=0. |

Historical class labels preserve the v1 discovery path. Current status/evidence cells state the default-mode behavior that W1-C may cite. v2 retro-validation uses multi-sentinel/stdout assertions plus permanent fixtures in `compiler/tests/empirical/`.

## Controlled coercions (10 entries — verified, NOT A4)
