...
<!-- Generated by scripts/render-excluded-features.py from master-plan.toml v23. -->
<!-- DO NOT HAND-EDIT this section. Edit master-plan.toml then run the script. -->

## Active promotion candidates

Active promotion candidates are tracked exclusively in ROADMAP Master Plan v23 §Phase A2 with formal subset predicates. Each requires lifecycle stage 1 (impact preflight) before assignment.
...
- affine ownership annotation subset (`x := affine T` and `affine T` type form, Core-typed; baseline: docs/language/LIVING_SPEC/01_keywords/linear_affine_annotation.vais — A2-06, reclassified A1→A2 v23 after empirical re-grep)

Subset definition format: `compiler/docs/certification/A2_SUBSETS.md` (Order step 9 deliverable).

## Removal queue (A4) — 6 verified entries
...
**Status**: v1-verified (single-sentinel discovery 2026-05-03); v2 retro-validation pending Order step 7. v17 (2026-05-04): A4-03/05/07 reclassified to Controlled per F-16/F-17/F-21 (IR-internal lowering, not user-facing source semantics).

Codemod dependency: Order step 2.

### A4-runtime-silent (4 entries) — type checker accepts; runtime produces wrong result
...
## Controlled coercions (12 entries — verified, NOT A4)
