...
<!-- Generated by scripts/render-excluded-features.py from master-plan.toml v311. -->
...
### A4-runtime-silent (10 entries) — type checker accepts; runtime produces wrong result
...
| A4-13 | Box<T> ↔ T auto-unwrap (call site) | `vais-types/checker_expr/calls.rs (Box<T> arg accepted where T expected)` | F take_i64(x: i64) -> i64 { R x }; F take_box(b: Box<i64>) -> i64 { R take_i64(b) } | E001 Type mismatch (Box<i64> is not i64) | historical: OK No errors found (silent accept); current default rejects with E001 |
...
| A4-03 | Auto-deref &T ↔ T (type-check silent path) | `vais-types/inference/unification.rs:754` | m: HashMap<u64, u64> = HashMap.new(); m.get_opt(&key) where key: u64 | E001 Type mismatch (&u64 is not u64) at TC layer | TC accepts; codegen lowers as i64 instead of ptr; clang catches loud: '%t6' defined with type 'ptr' but expected 'i64' at HashMap_get_opt$u64_u64 |
