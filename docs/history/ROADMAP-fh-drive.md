# Vais — Double Drive: struct-in-array-literal → Phase 4.x SCOPED (아카이브)

> **상태**: ✅ 완료 (2026-04-21). 3/3 task 완료 (F.1 codegen fix + H.1/H.2
>   doc findings). vaisdb cleanup drive 로 이관된 G.x 제외.
> **최종 baseline**: syntax=200, stages=14, std=82/82, living_spec=117,
>   phase158=18/18, vaisdb=237/261 — 0 regressions.
> **커밋 체인**: 195e5ad0 (F.1) → b82bd98b (H.1) → b74376b0 (H.2).
...
## 드라이브 결과 요약 (2026-04-21)

- **F.1** (compiler fix, Opus): `generate_array_expr` 에 Named struct 원소
  load 경로 추가. `v: Vec<Point> := [Point{x:1,y:2}, Point{x:3,y:4}]` 정상
  동작. LIVING_SPEC 116→117. LANGUAGE_SPEC Matrix L262 진전 (read OK,
  write-through는 B.4 memcpy 경로 별개 gap).
- **H.1** (investigation, Opus): E034 totality analyzer 는 정확하게 작동.
  vaisdb 4 E034 사례 모두 legitimate. 개선 방향 (flow-sensitive) 은 Phase
  4.x+ 스코프. `docs/totality-analysis-findings.md`.
- **H.2** (investigation, Opus): generic bound propagation TC 는 정상.
  bulk.vais 오류는 vaisdb 코드 cascade 로 결론. `docs/generic-bound-propagation-findings.md`.

주요 발견:
- 드라이브 중간 G 축 복잡도 재평가: vaisdb cleanup 은 단순 arg strip 이
  아니라 BufferPool API 재설계 (frame.data 흐름) 필요. 9+ 사이트 분석 복잡,
  vaisdb 자체 regression CI 부재 → 세션 내 완주 불가 판정. 스코프에서 이관.
- H 축 SCOPED 판단 원칙 적용: H.1/H.2 는 의심했던 compiler bug 가 실제로
  존재하지 않음을 확인 (minimal reproducer), vaisdb 쪽 이슈로 결론. 불필요한
  TC 변경을 피함 (Phase 158 요요 방지).

> **버전**: 2026-04-21 triple drive 시작 / 같은 날 완료 선언 (double drive 로 재정의)
...
iteration: 6
max_iterations: 25
strategy: H.3 drive finalize — sequential
opus_direct: H.3 drive-close = summarize + propose next-drive, judgment-only
...
- [x] H.2 — generic bound propagation 확인 (Opus direct) ✅ 2026-04-21
      changes:
        docs/generic-bound-propagation-findings.md (minimal reproducer 작성
          → 중첩 call 에서 bound propagate 정상 확인, bulk.vais 문제는
          vaisdb 코드 이슈로 결론)
      result:
        - 최소 재현: `W MyTrait { F greet() }` + `use_trait<T: MyTrait>` →
          `call_trait<T: MyTrait>` 체인 → main → 42 반환 ✓
        - TC 는 bound 를 정상 propagate 함
        - bulk.vais 의 E001 은 다른 vaisdb 에러 (E004 × 여러개 등) cascade
          일 가능성 높음 → vaisdb 코드 수정 범위
      scope 결론: TC 변경 없음. 별도 improvements (error position 정확도,
        cross-module re-export 체인) 은 future TC drive 후보로 기록.

- [x] H.3 — 드라이브 최종 완료 선언 (Opus direct) ✅ 2026-04-21
      changes:
        ROADMAP.md (이 파일을 fh-drive 아카이브로 이관 후 새 top-level
          ROADMAP 에 다음 드라이브 후보 5개 제안:
          1) Vec<Struct> write-through (B.4 후속) ⭐
          2) E034 flow-sensitive analysis
          3) 매크로 + dyn trait 완전 vtable
          4) Vec::new() in std/vec.vais
          5) vaisdb cleanup drive (별도 저장소))
      result:
        - double drive 3/3 완료, baseline 유지
        - 다음 드라이브 선택 대기

progress: 3/3 (100%) ✅ drive complete 2026-04-21
