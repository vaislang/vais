# Vais — Post-Compiler Drive: Cascade & Vec Completion (아카이브)

> **상태**: ✅ 완료 (2026-04-21). 4/4 task. Commit chain: `7c2e3c4b` (D.1) →
>   `57fb8992` (D.2) → `38d9a1f3` (D.3).
> **최종 baseline**: syntax=200, stages=14, std=82/82, vaisdb=237/261,
>   phase158=18/18 — 드라이브 시작 시점과 동일, 0 regressions.
> **버전**: 2026-04-21 신규 드라이브 시작 / 같은 날 완료 선언
...
- **D.1** (measurement, Opus): vaisdb=237/261 재측정, 24 failures를 (a/b/c) 버킷 분류 →
  `docs/vaisdb-cascade-survey.md`.
- **D.2** (compiler fix, Opus, user redirect 중간): Vec<T> literal lowering text-backend.
  `stmt_visitor.rs`에 5-field struct synthesis. `v: Vec<i64> := [...]` 정상 작동.
  LIVING_SPEC 112→116 (+3 reproducers).
- **D.3** (scope close, Opus): D.1 survey 재검토 결과 (a) 후보가 실은 (b) (vaisdb
  missing field). compiler-gap leverage 0 확인. 드라이브 스코프 narrow 종료.
- **E.1** (archive + handoff, Opus): 이 ROADMAP 아카이브, 다음 드라이브 후보 제안.

주요 발견:
- vaisdb 실패의 83.3% (20/24) 가 vaisdb-side bugs (missing field/method/arg count).
  compiler work 로 더 이상 leverage 없음 → 다음 드라이브는 vaisdb cleanup 또는 다른 축.
- 작업 중 발견된 추가 compiler gap: **struct-in-array-literal**
  (`[Point{...}, ...]` → `store %Point <ptr>`). vaisdb 사용 0, stdlib 사용 0이지만
  D.2 Vec 리터럴 완성도의 자연스러운 후속.
- `cmd_run` / default `vaisc build` 는 text backend 를 사용한다는 사실이 ROADMAP
  작성 시점에 명시되지 않아 D.2 범위가 inkwell 로 잘못 향해 있었음. 중간 발견 후
  user redirect 로 text backend 로 전환.
...
iteration: 4
max_iterations: 15
strategy: E.1 drive-close — sequential
opus_direct: E.1 drive-close requires summarizing findings + proposing next-drive candidates, inseparable from judgment
...
- [x] E.1 — 드라이브 완료 선언 + 다음 드라이브 후보 제안 (Opus direct) ✅ 2026-04-21
      changes:
        ROADMAP.md (이 파일을 cascade-drive 아카이브로 이관 후 새 top-level
          ROADMAP.md 에 다음 드라이브 후보 3개 제안:
          1) vaisdb cleanup drive (vaisdb 저장소 쪽, 20 bugs),
          2) struct-in-array-literal text-backend fix (D.2 자연 후속),
          3) Phase 4.x SCOPED 심층 (E034 purity + generic bounds))
      result:
        - 드라이브 4/4 완료, 모든 baseline 유지
        - 다음 드라이브 선택 대기
      blockedBy: D.3 (completed)
...
progress: 4/4 (100%) ✅ drive complete 2026-04-21
