...
# 연산
value? -> T                # unwrap (nil/err 전파)
value ?? default -> T      # 기본값으로 unwrap
value! -> T                # unwrap 또는 패닉
```
...
# 디렉토리 작업
fs.mkdir("new_dir")?                         # 생성
fs.mkdir_all("a/b/c")?                       # 재귀적 생성
fs.rmdir("dir")?                             # 삭제
fs.rmdir_all("dir")?                         # 재귀적 삭제
...
println("소요 시간: " + elapsed.as_ms().str + "ms")
```
...
random.choice([1, 2, 3, 4, 5])               # 무작위 선택
random.shuffle([1, 2, 3, 4, 5])              # 셔플
...
assert_lt(a, b)                              # a < b
assert_gt(a, b)                              # a > b
assert_some(option)                          # some
assert_none(option)                          # nil
assert_ok(result)                            # ok
assert_err(result)                           # err
```
