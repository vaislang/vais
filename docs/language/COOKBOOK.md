...
## 23. `Vec<Struct>[i].field = expr` — Phase 3.14 codegen gap

```vais
S Point { x: i64, y: i64 }

# ❌ DON'T — codegen 미지원 (C005)
v[0].x = 99

# ✅ DO — 읽고 수정하고 쓰기 (read-modify-write)
p := v[0]
p.x = 99
v[0] = p
```

**왜 실패하는지**: LLVM GEP 복합 주소 계산 (index GEP + field GEP + store) 경로가 codegen에 없음. Phase 3.14 본격 구현 대기 중.

**참조**: `docs/CODEGEN_FEATURES.md` §5 Vec<Struct>[i].field write
