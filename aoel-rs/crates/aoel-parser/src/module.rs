...
use aoel_ast::{Item, Program, UseDef};
...
impl ModuleResolver {
    ...
    fn resolve_path(&self, path: &[String]) -> Result<PathBuf, ParseError> {
        ...
        // Try without extension (might be a directory with mod.aoel)
        file_path.set_extension("");
        file_path.push("mod.aoel");
        if file_path.exists() {
            return Ok(file_path);
        }
        ...
        Err(ParseError::ModuleNotFound {
            ...
            reason: format!(
                ...
                self.base_dir.join(path.join("/")).with_extension("aoel").display(),
                self.base_dir.join(path.join("/")).join("mod.aoel").display()
            ),
            span: aoel_lexer::Span::default(),
        })
    }
