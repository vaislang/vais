//! C Code Generator
//!
//! Generates C code from compiled Vais functions
...
impl CCodeGenerator {
    ...
    fn generate_runtime(&mut self) {
        // Value 타입 정의
        self.writeln("// Vais Runtime Types");
    ...
    fn generate_instructions(&mut self, instructions: &[vais_ir::Instruction], func_name: &str) -> CodegenResult<()> {
        let mut i = 0;
    ...
    fn sanitize_name(&self, name: &str) -> String {
        ...
        format!("vais_{}", sanitized)
    }

    /// 인라인 명령어 생성 (Map/Filter 내부용)
    /// 레이블 없이 명령어만 생성
    fn generate_inline_instructions(&mut self, instructions: &[vais_ir::Instruction], _func_name: &str) -> CodegenResult<()> {
...
mod tests {
    ...
    fn test_simple_function() {
        ...
        assert!(code.contains("static Value vais_add"));
    ...
    fn test_factorial() {
        ...
        let code = generate_c(&[func]).unwrap();
        assert!(code.contains("static Value vais_fact"));
        assert!(code.contains("vais_fact(")); // recursive call
    }
