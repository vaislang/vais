...
| `Map<Str,Char>` | Verified for local values, local assignment copy, parameter reference/mutation, `insert`, `remove`, `clear`, `get`, `get_opt`, `contains`, and `len`; returns remain gated |
...
`Map<Str,V>` return values, `Map<Str,Char>` return values, generic Map return
values, iteration, `Result`, hashing controls, or map literals with entries.
