...
| `fn make() -> Map<Int,Int>` / `let m: Map<Int,Int> = make()` / `fn make() -> Map<Int,Bool>` / `let m: Map<Int,Bool> = make()` | Return a verified concrete Map into caller-owned local storage |
...
The slice does not include generic key/value lowering, `Map<Int,Char>` return
values, generic Map return values, iteration, deletion,
