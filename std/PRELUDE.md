...
| `Map<Str,Char>` | Verified for local values, local assignment copy, `insert`, `remove`, `clear`, `get`, `get_opt`, `contains`, and `len`; parameters and returns remain gated |
...
| `m.get_opt(key)` | Return `Some(value)` for a present key or `None` when absent on `Map<Int,Int>`, `Map<Int,Bool>`, `Map<Int,Char>`, `Map<Str,Int>`, `Map<Str,Bool>`, or `Map<Str,Char>` |
...
`Map<Str,V>` return values, `Map<Str,Char>` parameters or return values,
generic Map return values, iteration, `Result`, hashing controls, or map
literals with entries.
