...
| `fn make() -> Map<Int,Int>` / `let m: Map<Int,Int> = make()` / `fn make() -> Map<Int,Bool>` / `let m: Map<Int,Bool> = make()` / `fn make() -> Map<Int,Char>` / `let m: Map<Int,Char> = make()` / `fn make() -> Map<Str,Int>` / `let m: Map<Str,Int> = make()` / `fn make() -> Map<Str,Bool>` / `let m: Map<Str,Bool> = make()` | Return a verified concrete Map into caller-owned local storage |
...
`Map<Str,V>` return values, generic Map return values, iteration, `Result`,
hashing controls, or map literals with entries.
