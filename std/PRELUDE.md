...
| `fn f(m: Map<Int,Int>) -> Int` / `fn f(m: Map<Int,Bool>) -> Int` | Pass a Map by reference so the callee can read or mutate the caller-visible map |
...
The slice does not include generic key/value lowering, `Map<Int,Char>`
function parameters, return values, iteration, deletion, `Result`, hashing
controls, or map literals with entries. Unverified `Map<Int,Char>` Map
