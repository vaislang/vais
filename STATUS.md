...
| C9 | `?` postfix type on user-defined struct (e.g. `-> SpinLockGuard?`) generates function signature returning i64 but body returns Option<T>, producing IR verifier error "ret type mismatch". | `F try_lock(&self) -> SpinLockGuard?` | use explicit `Option<SpinLockGuard>` instead of `?` postfix | n/a — unblocks C4 once fixed |
