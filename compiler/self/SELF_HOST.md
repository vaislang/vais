...
| `fixpoint_full.nl` (FP10f–FP12m) | **the unified compiler**: functions (0-4 params, recursion, nested calls) + mutable vars + while + if/else + arrays + Lists + structs + **strings** (`s[i]`/`s.len()`) + putchar + **comparison-as-value** + **`(...)` grouping** + **`>=`/`<=`** + **`and`/`or` as values** + **compound conditions** + **string parameters** (`fn tokenize(s: Str)` — i8* param, byte-load, **runtime `s.len()`**; a function can take the source as a string param and scan it — the actual self-host tokenizer signature) — every core construct in one program | `test-fixpoint-full.sh` |
