...
| `fixpoint_full.nl` (FP10f–FP12n) | **the unified compiler**: functions (0-4 params, recursion, nested calls) + mutable vars + while + if/else + arrays + Lists + structs + **strings** (`s[i]`/`s.len()`) + putchar + **comparison-as-value** + **`(...)` grouping** + **`>=`/`<=`** + **`and`/`or` as values** + **compound conditions** + **string parameters** (`fn tokenize(s: Str)`) + **List parameters** (`fn eval(toks: List<Int>)` — passed by buffer pointer, `xs[i]`/`xs.len`; the parser/evaluator signature) — every core construct in one program | `test-fixpoint-full.sh` |
