...
| `fixpoint_full.nl` (FP10f–FP12p) | **the unified compiler**: functions (**0-8 params**, recursion, nested calls, **bare call statements**) + mutable vars + while + if/else + arrays + Lists + structs + **strings** (`s[i]`/`s.len()`) + putchar + **comparison-as-value** + **`(...)` grouping** + **`>=`/`<=`** + **`and`/`or` as values** + **compound conditions** + **string parameters** + **List parameters** (read + **push/write-through**) — runs the REAL self-host helpers `name_eq`/`kw3`, and a **full out-param tokenizer** `fn tokenize(src: Str, out: List<Int>)` that scans the source and fills the output List (the caller reads the tokens) | `test-fixpoint-full.sh` |
