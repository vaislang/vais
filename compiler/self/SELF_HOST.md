...
| `fixpoint_full.nl` (FP10f–FP12o) | **the unified compiler**: functions (**0-8 params**, recursion, nested calls) + mutable vars + while + if/else + arrays + Lists + structs + **strings** (`s[i]`/`s.len()`) + putchar + **comparison-as-value** + **`(...)` grouping** + **`>=`/`<=`** + **`and`/`or` as values** + **compound conditions** + **string parameters** (`fn tokenize(s: Str)`) + **List parameters** (`fn eval(toks: List<Int>)`) — runs the REAL self-host helpers `name_eq` (5 params) and `kw3` (6 params); the core `gen_stmts`/`gen_expr` take 8 | `test-fixpoint-full.sh` |
