...
| `fixpoint_full.nl` (FP10f–FP12g) | **the unified compiler**: functions (0-4 params, recursion, nested calls) + mutable vars + while + if/else + arrays + Lists + structs + **strings** (`s[i]`/`s.len()`) + putchar + **comparison-as-value** (`return a == b` → icmp + zext) — every core construct in one program | `test-fixpoint-full.sh` |
