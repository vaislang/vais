...
`nl`은 repo 경로와 현재 체크아웃된 파일 확장자가 안정된 게이트에 묶여 있기 때문에 전환기 구현 코드명으로 유지한다.
트랜스파일러의 canonical 이름은 `legacy_vais_bootstrap.py`로 바꿨고, 기존 `nl2vais.py`는 호환 래퍼로만 남긴다.
`.vais` 확장자 전환 가능성은 `scripts/test-vais-extension-migration.sh`로 검증한다.
...
| 2. 파일 확장자 | `.nl` checked-in, `.vais` gate 지원 | physical rename만 남음 |
...
- `.nl` corpus를 `.vais` mirror로 복사한 migration gate를 추가했다.
...
git mv nl vais                                 # 1. 폴더
cd vais
# 2. 확장자 변경: examples/와 compiler/self/ 등 source 파일 .nl → .vais
find examples compiler/self -name '*.nl' -print0 | while IFS= read -r -d '' f; do
  git mv "$f" "${f%.nl}.vais"
done
# 3. manifest/scripts/docs의 실제 경로를 .vais로 전환하고 gate 재실행
...
## 주의
- 확장자 `.nl`은 현재 checked-in 파일 경로의 일부다. `.vais` mirror gate는 통과했지만, physical rename은 별도 커밋으로 진행해야 한다.
