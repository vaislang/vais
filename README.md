...
- `scripts/vaisc` native front 계약 추가: Int 함수/let/return/if/while/plain call/print/putchar,
  simple struct, payload-free enum/match, small Int-coded payload enum/match, List push/len/index/sum subset은 통과하고
  for/broader payload enum/match/string/unsupported method/Rust식 `&&` 등은
...
- native parity manifest/gate 추가: `tools/vaisc-parity.tsv`가 예제와 self-host tier를
  ...
  native-supported 항목을 Legacy bootstrap oracle과 값 비교한다.
