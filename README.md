...
├── compiler/
│   └── transpiler/      New Vais(.vais/.nl) → Legacy Vais adapter (bootstrap backend)
│       ├── legacy_vais_bootstrap.py
│       └── nl2vais.py  compatibility wrapper
├── examples/            .vais 예제 = New Vais cold-start 코퍼스 (P9 인프라)
├── tools/
│   ├── vaisc.py         New Vais compiler CLI 구현
│   └── vais-check.py    P4 lint/error-help 도구 (`nl-check.py`는 wrapper)
├── tests/               트랜스파일러/vais-check 단위 테스트
└── scripts/
...
- `scripts/vaisc` native front 계약 추가: Int 함수/let/return/if/while/plain call/print/putchar,
  simple struct, payload-free enum/match, small Int-coded payload enum/match, single-Int closure return,
  List push/len/index/sum subset은 통과하고
  for/broader payload enum/match/broader closure/string/unsupported method/Rust식 `&&` 등은
  source 좌표와 `help:` 진단으로 거절(`scripts/test-vaisc-front.sh`).
...
- native parity manifest/gate 추가: `tools/vaisc-parity.tsv`가 예제와 self-host tier를
  `native-supported`/`bootstrap-only`/`tracked`로 기록하고, `scripts/test-vaisc-parity.sh`가
  native-supported 항목을 Legacy bootstrap oracle과 값 비교한다. `compiler/self/*` tier는
  검증된 compiler source로 제품 bootstrap engine에서 직접 받는다.
...
scripts/test-vais-extension-migration.sh                 # .vais corpus의 transitional .nl 호환성 검증
scripts/build.sh examples/c4.vais -o /tmp/c4              # Legacy bootstrap oracle 경로
```
