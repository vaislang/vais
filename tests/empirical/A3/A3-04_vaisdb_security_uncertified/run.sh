#!/usr/bin/env bash
# A3-04 — vaisdb security (TLS / encryption / RBAC)
...
if [[ "$EXIT" == "0" ]]; then
  echo "DRIFT: A3-04 strict-mode check now succeeds — surface may be certified" >&2
  exit 1
fi

for pat in "E_IMPORT_NOT_FOUND" "security"; do
  if ! grep -qF "$pat" <<< "$OUT"; then
    echo "DRIFT: A3-04 strict-mode check failed but stderr lacks '$pat':" >&2
    echo "$OUT" >&2
    exit 1
  fi
done

echo "A3-04 OK: VAIS_STRICT_IMPORTS=1 rejects U vaisdb/security/types with E_IMPORT_NOT_FOUND."
