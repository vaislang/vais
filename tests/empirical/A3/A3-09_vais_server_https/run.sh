...
VAIS_STRICT_IMPORTS=1 "$VAISC" check "$WORK/probe.vais" >/dev/null 2>&1 \
  && EXIT=0 || EXIT=$?
if [[ "$EXIT" == "0" ]]; then
  echo "DRIFT: A3-09 strict-mode check now succeeds — surface may be certified" >&2
  exit 1
fi
for pat in "E_IMPORT_NOT_FOUND" "https"; do
  if ! grep -qF "$pat" <<< "$OUT"; then
    echo "DRIFT: A3-09 strict-mode check failed but stderr lacks '$pat':" >&2
    echo "$OUT" >&2
    exit 1
  fi
done
echo "A3-09 OK: VAIS_STRICT_IMPORTS=1 rejects U server/https/handler with E_IMPORT_NOT_FOUND."
