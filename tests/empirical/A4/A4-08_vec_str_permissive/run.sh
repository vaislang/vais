#!/usr/bin/env bash
# A4-08 — Vec<T>↔&T permissive (post-removal: check_fails form).
...
if [[ "$EXIT" == "0" ]]; then
  echo "DRIFT: A4-08 check now succeeds — strict default may have regressed." >&2
...
if ! grep -qE "E001|Type mismatch|expected" <<< "$OUT"; then
  echo "DRIFT: A4-08 check failed but stderr lacks expected E001 pattern:" >&2
  echo "$OUT" >&2
  exit 1
fi

echo "A4-08 OK: vaisc check rejects probe with E001 (silent surface removed; default-mode strict)."
