#!/usr/bin/env bash
# A4-09 — Lifetime ref erasure (post-removal: check_fails form).
...
if [[ "$EXIT" == "0" ]]; then
  echo "DRIFT: A4-09 check now succeeds — strict default may have regressed." >&2
...
if ! grep -qE "E001|Type mismatch|expected" <<< "$OUT"; then
  echo "DRIFT: A4-09 check failed but stderr lacks expected E001 pattern:" >&2
  echo "$OUT" >&2
  exit 1
fi

echo "A4-09 OK: vaisc check rejects probe with E001 (silent surface removed; default-mode strict)."
