...
printf '(define i 0)\n(define s "x")\n(while (< i 5000) (set s (str-cat "x" "y")) (set i (+ i 1)))\n(define d 0)\n(define t "")\n(while (< d 5000) (set t (num->str d)) (set d (+ d 1)))\n(define c 0)\n(define l nil)\n(while (< c 5000) (set l (cons c l)) (set c (+ c 1)))\n(if (= s "xy") (if (= t "4999") (if (= (car l) 4999) 42 1) 2) 3)\n' > "$tmp/lisp-pool-stress.lisp"
expect_exit "vaislisp deduped partitioned pools survive hot loops" 42 /bin/sh -c "'$lisp_dist/bin/vaislisp' '$tmp/lisp-pool-stress.lisp' >/dev/null"
...
expect_exit "vaisjq quoted keys reach odd fields" 0 /bin/sh -c "printf '{\"user name\":\"ann\"}' | '$jq_dist/bin/vaisjq' '.[\"user name\"]' | grep -qx '\"ann\"'"
expect_exit "vaisjq slices clamp and count from end" 0 /bin/sh -c "printf '[10,20,30,40]' | '$jq_dist/bin/vaisjq' -c '.[-2:]' | grep -qx '\[30,40\]'"
...
# vaiscalc: the integer expression calculator — self-test, precedence,
# parentheses, unary minus, and the loud error exits.
calc_dist="$tmp/vaiscalc-dist"
rm -rf "$calc_dist"
expect_exit "vaiscalc package build" 0 "$ROOT/scripts/vaisc" package "$ROOT/examples/e399_vaiscalc_package" -o "$calc_dist"
expect_exit "vaiscalc package self-test" 42 /bin/sh -c "'$calc_dist/bin/vaiscalc' < /dev/null"
expect_exit "vaiscalc precedence and parens" 0 /bin/sh -c "'$calc_dist/bin/vaiscalc' '2 * (3 + 4) * 3' < /dev/null | grep -qx '42'"
expect_exit "vaiscalc unary minus and modulo" 0 /bin/sh -c "'$calc_dist/bin/vaiscalc' '-(10 - 52) + 100 % 29 - 13' < /dev/null | grep -qx '42'"
expect_exit "vaiscalc division by zero rejects loud" 3 /bin/sh -c "'$calc_dist/bin/vaiscalc' '4 / 0' < /dev/null >/dev/null 2>&1"
expect_exit "vaiscalc trailing garbage rejects loud" 3 /bin/sh -c "'$calc_dist/bin/vaiscalc' '7 seven' < /dev/null >/dev/null 2>&1"
...
cp "$calc_dist/bin/vaiscalc" "$vbox_bin/vaiscalc"
...
expect_exit "vaisbox list count" 0 /bin/sh -c "'$vbox_bin/vaisbox' list | wc -l | grep -q 16"
...
expect_exit "vaisbox dispatch vaiscalc" 0 /bin/sh -c "'$vbox_bin/vaisbox' vaiscalc '6 * 7' < /dev/null | grep -qx '42'"
