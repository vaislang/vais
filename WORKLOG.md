...
+1) 대신 **pool_put 내용 dedup** — str-cat 등 계산 문자열의 루프
재-intern까지 한 번에 해소, 파라미터 불변, 스캔 비용은 distinct
문자열 수에만 비례(핫루프=distinct 극소가 바로 그 케이스). 동일
내용→동일 태그 값은 문자열 의미상 안전(= 비교는 내용 기준, 오히려
identity==content로 강화).

실증: 파티션(8190)마저 넘던 원래 부하(5000회 str-cat = 15000
intern)가 **3슬롯**으로, 5000 distinct(num->str 0..4999)는 오버플로
파티션을 실사용(5001슬롯, dedup 스캔 ~12.5M 비교 포함 0.06s), 5000
cons 병행 — 강화 스트레스로 게이트 교체.
...
배열/객체 **공용 (key,val,next) 셀 힙**(배열은 key=-1)이라 파서/렌더러
각 1개가 양 형태를 서빙. 재귀 하강 파서는 (value,next) 튜플, 이스케이프
6종, 비정수·\u·트레일링 LOUD. 필터는 세그먼트 리스트(kind+payload)로
컴파일 후 재귀 방출 — [] 반복이 다중 출력, 결측은 null 체이닝(jq 관용),
타입 오류 exit 3. pretty 2-space 기본/-c compact.

**경유 root-fix 1 — 파라미터 10→16**: eval_filter(12파라미터)가 트리거.
full의 Fn 모델이 p0..p9 하드코딩 — 11번째부터 마지막 슬롯 덮어쓰기로
본문 이름 미해결(%v-1 GEP). 격리 프로브(p11/p12)로 확정 후 Fn 구조체
+파서 필 2체인+SSA 헤더+슬롯 등록+**호출측 3사이트**(a0k..a9k/ls/li/lb
병렬 체인) 전부 16으로. front는 17+ LOUD 거부. 55필드 구조체 프로브로
Fn 확장 안전성 선검증. 트랩: 확장 후 P16 프로브 105 실패는 **별개
버그**(아래)의 위양성 — 시드가 음수 리터럴이었음.

**경유 root-fix 2 — 단항 마이너스**: `let x = -5`가 full에서 silent
0(코퍼스 전체가 `0 - n` 관용이라 414 예제가 못 잡음), direct는 정상 —
값 발산. gen_factor에 kind 4 분기: 리터럴은 상수 접기, 그 외(-x/-f()/
리스트 원소)는 `sub i64 0,` 로워링 + skip_factor 미러. 발견 경로가
교훈: 16-param 프로브의 음수 시드가 우연히 표면화 — **프로브 시드도
검증 표면 안에서 골라야 위양성이 없다**.

core 재생성 2회 전부 gen1==gen2 즉시 수렴. e397/e398 잠금(양 엔진 42),
front 17-param 픽스처, 워크플로 vaisjq 12케이스, vaisbox 로스터 15
(15*3-3=42), parity +3행.
...
밴드=세션 풀 인덱스, CONS 밴드=병렬 car/cdr 힙, FN 밴드=defun/lambda
테이블, NIL 상수. 밴드 밖 산술 LOUD. 신규 폼: "..." 리터럴 +
str-len/str-cat/= 동등, cons/car/cdr/list/null?/nil + 재귀 리스트 렌더
"(1 2 3)", **lambda 값-호출 적용 경로**(head 평가 → FN이면 적용) — 고차
합성 (twice (lambda (n) (* n 3)) 4)=36, 즉시호출, 파라미터로 fn 전달
전부 개통. self-test 13~23 + 워크플로 12케이스.

**누수 프로브가 캡을 먼저 발견**: 20k줄 세션 → 토큰 리스트 4095 캡
trap(~340줄, LOUD 134). 무-정의 라인의 토큰 pop-롤백(문자열/cons는
자기 풀에 interned라 값 생존, fnames.len() 불변 판정) 후 **20k줄
0.42s/피크 RSS 10.4MB** — 라인당 ~500B 아레나 누적 = 실사용 무해 실측.
풀/힙 캡은 세션 한계로 문서화(파티션/GC는 수요 시).

**경유 컴파일러 작업 2건**: ① 튜플 로워링 **2-pass화** — 단일 전방
패스라 앞선 함수가 뒤에 선언된 튜플 fn을 구조분해로 못 불렀음(apply/
eval 상호재귀가 수요). 시그니처 프리패스 수집(unknown-call 2-pass
전례), e394에 전방-참조 케이스 확장. ② 한줄 식-본문 fn(`fn f() -> Int
{ 64 }`)이 full 엔진에서 **빈 IR 방출**(front 미거부, clang에서 늦게
표면) — 후보 등록, vaislisp는 검증 멀티라인 표기로 전환. 트랩: 파이프
tail/head 뒤 $? 오인 4회차 — 빌드 exit은 로그 파일 리다이렉트로.
...
qcount × rarity × min(dcount, 8), rarity = bit_len(N/df) clamp≥1
(df = 텀 매치 문서 수 — 스캔의 tscores.len()이 공짜 제공, N = 등록
문서 수, 고아 포스팅으로 df>N 가능해 clamp). df=0 텀은 skip(쿼리 자연
전멸). 캡 8=2^3, rarity ≤ ~10이라 오버플로 무관.

repo 실코퍼스: **str_slice_raw 스프레드 6,633-vs-98 → 전-텀 실보유자
동점 밴드(80)** — common-token 물량 지배 소멸(목표 달성). token_shard_
at은 정의 모듈 5위 97점 — 상위 4개는 그 식별자를 다량 기술한 워킹
노트/게이트 파일이라 bag-of-words로 방어 가능. **추가 품질(정의-사이트
우대 등)은 릴레번스 하네스(질의-정답셋)가 필요 — 수요 대기 후보.**

트랩: 픽스처 기대값 손계산 오답 — self-test 86을 N=3으로 계산했으나
실제 레지스트리는 d3("vector vector cache" — cache 보유!)+huge 포함
4문서 → d1=8. 체크 발화(got=86)가 즉시 교정 — **수치 픽스처는 손계산
말고 실행이 판정**, 인덱스 상태는 해당 시점 전체 시퀀스로 복원할 것.
...
구성, e337 vaisdb를 노출 — **소형 코퍼스조차 ingest-dir 즉사**(v1.1.0
메시지 trap이 정확히 발화, exit 134: 지난주 승격한 진단이 제품 실측에서
바로 값함).

합성 문서로 차원 분리(고유 term × 총 단어 × 라인 × 문서 수):
**Map 256 계약이 이중 즉사점** — ① 문서당 고유 term 257에서 trap(실전
문서는 examples/README 실측 1,597 = 6배 초과) ② 인덱스 flat key가 문서
2개째(400키)에서 trap. 단어·라인은 스트리밍이라 5,000에서도 무제한,
성능도 무결(ingest 10ms/문서). 부수 발견: trap 시 인덱스 부분 잔존
(ingest 원자성 부재). 리포트 docs/design/VAISDB-SCALE-BASELINE.md.

판정: 어휘는 무한 성장하므로 Map cap 상향만으론 정본이 못 됨 — **병행
전략**: 컴파일러 수요 주도 1호 = Map 계약 256→4096(실용 구간 확보,
P1) + VaisDB 디스크-우선 인덱스 재설계(term-샤딩 포스팅 파일 +
fs_append_text — 도구 사이클의 스트리밍 선례와 신규 API가 정확히 여기에
꽂힘, P2) + 원자성(P3) + vaisbench 스케일 게이트(P4).

**다음 세션:** P1(Map 계약 상향 — Map insert/key_at/value_at cap 지점은
2026-07-18b·26 trap 스프린트에서 이미 전수 파악됨) 착수.
...
경유 이슈(자기 교훈): 첫 게이트 케이스를 `set -e` 아래 bare 파이프라인으로
써서 매칭 수 exit(2)에 workflow 게이트가 즉사(LADDER-EXIT 2) — 래더 확인
전 커밋한 실수 포함. `/bin/sh -c` 형태로 통일 후 재검증 GREEN.
...
69→36분) ② fixpoint-full 무상태 해시 샤딩(863→320s) ③ value/parity 샤딩
(각 206/205→129s, 정본 카운트 합산 재구성) ④ selfhost 페이즈 병렬화
(272→177s, 독립성 판정이 핵심). **최종 래더 ~22분(3.1×)**, 전 유닛에서
커버리지 불변(카운트/케이스-집합/diff) 실증. 공통 패턴: 무상태 해시
버킷(케이스명) 또는 독립 페이즈 팬아웃 + 직렬 경로 env 보존 + 셸 집계.

잔여는 수확 체감(최중량 직렬 프로브·프로세스 스폰 하한)으로 종결.
PERF-BASELINE.md가 기준선·이력·재개 트리거를 보관.
