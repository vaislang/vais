...
impl Backend {
    ...
    fn get_completions_at(&self, uri: &Url, position: Position) -> Vec<CompletionItem> {
        ...
        if let Ok(program) = vais_parser::parse(&text) {
...
impl LanguageServer for Backend {
    async fn initialize(&self, _: InitializeParams) -> Result<InitializeResult> {
        Ok(InitializeResult {
            ...
            server_info: Some(ServerInfo {
                name: "Vais Language Server".to_string(),
    ...
    async fn initialized(&self, _: InitializedParams) {
        self.client
            .log_message(MessageType::INFO, "Vais Language Server v0.2.0 initialized")
            .await;
    }
