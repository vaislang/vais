...
Browser-based Vais code editor and runner using WebAssembly.
...
- `execute(source: string) -> string` - Execute Vais code, returns JSON result
- `check(source: string) -> string` - Parse and type-check code
- `format_code(source: string) -> string` - Format Vais code
...
```javascript
import init, { execute, format_code } from './pkg/vais_playground.js';
